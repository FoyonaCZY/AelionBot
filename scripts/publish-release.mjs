...
const merged={...mac[1],files:mac.flatMap(m=>m.files)};writeFileSync(join(directory,'latest-mac.yml'),stringify(merged));
const assets=[...new Set([...required.filter(f=>!f.endsWith('-verification.json')&&!/^latest-mac-(arm64|x64)\.yml$/.test(f)),'latest-mac.yml',...readdirSync(directory).filter(f=>f.endsWith('.blockmap'))])];
...
gh(['release','edit',tag,'--draft=false','--latest']);console.log(`Published ${tag} with Windows, macOS arm64 and macOS x64 assets`);
