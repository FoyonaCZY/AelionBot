...
try{
 ...
 const windowVisible=async(pattern:string)=>{const result=await vm.executeDesktop(`timeout 90 sh -c 'until xdotool search --onlyvisible --class "${pattern}"; do sleep 2; done'`,'mac-smoke',AbortSignal.timeout(120000));if(result.exitCode!==0)throw Error('Application window unavailable: '+pattern);return result.stdout;};
 ...
 const officeScreen=await computer.execute('mac-smoke',{action:'screenshot'},AbortSignal.timeout(120000));writeFileSync(join(out,`mac-${arch}-office.png`),readFileSync(join(computer.imageDir,officeScreen.screenshot.id+'.png')));
