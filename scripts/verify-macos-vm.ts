...
try{
 ...
 const deadline=Date.now()+35*60000;let reboots=0,nextProgress=Date.now()+60000;
 while(Date.now()<deadline){await vm.refresh();if(vm.state.needsReboot){if(++reboots>2)throw Error('Desktop did not settle after reboot');await vm.restart();}if(vm.state.desktopReady&&vm.state.appsReady&&!vm.state.maintenance)break;if(vm.state.lastError&&!vm.state.maintenance)throw Error(vm.state.lastError);if(Date.now()>=nextProgress){nextProgress=Date.now()+60000;try{await preparationProgress();}catch(error){console.log('Guest preparation probe failed: '+(error as Error).message);}}await new Promise(r=>setTimeout(r,5000));}
