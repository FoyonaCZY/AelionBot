...
try{
 ...
 try{logs['desktop-install.log']=redact((await vm.execute('tail -n 2200 /var/log/aelion-desktop.log','mac-smoke',AbortSignal.timeout(20000))).stdout);const lines=logs['desktop-install.log'].split('\n'),important=new Set<number>();for(let i=0;i<lines.length;i++)if(/error|failed|fault|illegal instruction|dependency problems/i.test(lines[i]))for(let at=Math.max(0,i-4);at<=Math.min(lines.length-1,i+5);at++)important.add(at);console.error('Desktop installation errors:\n'+[...important].sort((a,b)=>a-b).map(i=>lines[i]).join('\n').slice(-24000));}catch{}
