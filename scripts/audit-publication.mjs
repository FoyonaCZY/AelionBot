...
for(const file of [...new Set(files)]){
  ...
  if(!textExtensions.has(extname(file))&&!['.gitignore','.gitattributes','LICENSE'].includes(file)){if(!/^assets\/icon\.(png|ico)$/.test(file)&&!/^docs\/assets\/(?:screenshots|product)\/[^/]+\.(png|jpe?g|webp)$/i.test(file)&&!/^website\/public\/blog-media\/(?:[^/]+\/)*[^/]+\.(png|jpe?g|webp|gif)$/i.test(file))findings.push({file,kind:'unexpected-artifact'});continue;}
