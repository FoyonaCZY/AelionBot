...
const contents=new Map(),textExtensions=new Set(['.ts','.tsx','.mts','.cts','.js','.mjs','.cjs','.json','.md','.html','.txt','.yml','.yaml','.ps1','.cmd','.css','.svg','.conf','.plist','.py','.sh','.csv']);
...
for(const file of [...new Set(files)]){
  ...
  if(!textExtensions.has(extname(file))&&!['.gitignore','.gitattributes','LICENSE'].includes(file)&&!/(?:^|\/)(?:LICENSE|NOTICE)$/i.test(file)){if(!/^assets\/icon\.(png|ico)$/.test(file)&&file!=='assets/wallpaper-light.png'&&!/^src\/assets\/[^/]+\.(png|jpe?g|webp)$/i.test(file)&&!/^assets\/design-systems\/.+\.(png|jpe?g|webp|gif)$/i.test(file)&&!/^docs\/assets\/(?:screenshots|product)\/[^/]+\.(png|jpe?g|webp)$/i.test(file)&&!/^website\/public\/blog-media\/(?:[^/]+\/)*[^/]+\.(png|jpe?g|webp|gif)$/i.test(file))findings.push({file,kind:'unexpected-artifact'});continue;}
