...
export class ModelClient {
 ...
 async complete(messages:WireMessage[],tools:ToolDefinition[],signal:AbortSignal,onText:(text:string)=>void=()=>{},options:CompletionOptions={}):Promise<Completion>{
  ...
  let measuredMessages:WireMessage[]|undefined;
  for(let attempt=0;;attempt++){
   ...
   try{
    ...
    signal.throwIfAborted();const result=accumulator.result();if(imagesOmitted)result.inputImagesOmitted=true;
