...
export function hostShell(command:string,env:NodeJS.ProcessEnv=process.env,platform=process.platform){
 ...
 return {executable:win32.join(env.SystemRoot||env.SYSTEMROOT||'C:\\Windows','System32','WindowsPowerShell','v1.0','powershell.exe'),args:['-NoLogo','-NoProfile','-NonInteractive','-EncodedCommand',Buffer.from(script,'utf16le').toString('base64')],detached:false};
}
