...
import {existsSync,mkdirSync,readFileSync,realpathSync,statSync,writeFileSync,renameSync,unlinkSync} from 'node:fs';
...
export interface HostOptions {dataDir:string;projectDir:string;homeDir:string;env?:NodeJS.ProcessEnv;secrets?:()=>string[];}
const forbidden=/[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f\u202a-\u202e\u2066-\u2069]/;
...
export class HostComputer {
  ...
  context(botId:string){return {platform:process.platform,shell:'PowerShell',homeDir:this.options.homeDir,projectDir:this.options.projectDir,workspace:this.workspace(botId)};}
  ...
  async execute(botId:string,runId:string,args:Record<string,unknown>,signal:AbortSignal){
    ...
    const defaultCwd=this.workspace(botId),cwd=this.canonical(args.cwd===undefined?defaultCwd:this.path(args.cwd));
  ...
  async readFile(botId:string,runId:string,args:Record<string,unknown>,signal:AbortSignal){
    const path=this.canonical(this.path(args.path)),reason=text(args.reason,'reason',1000);
  ...
  async writeFile(botId:string,runId:string,args:Record<string,unknown>,signal:AbortSignal){
    const path=this.canonical(this.path(args.path)),reason=text(args.reason,'reason',1000);
