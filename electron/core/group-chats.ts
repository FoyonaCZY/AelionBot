...
export class GroupChats implements GroupGateway {
  ...
  private append(room:GroupRoom,sender:GroupSender,content:string,round?:GroupRound,mentions?:BotMention[],kind:import('../../src/group-types').GroupMessage['kind']=sender.kind==='system'?'system':'message',replyTo?:string,extra:Partial<Pick<import('../../src/group-types').GroupMessage,'reaction'|'runIds'|'event'|'attachments'|'scheduled'>>={}){
    const message:import('../../src/group-types').GroupMessage={id:randomUUID(),seq:(room.messages.at(-1)?.seq||0)+1,groupId:room.id,sender,kind,content,time:now(),rootId:round?.id,mentions,replyTo,...extra};room.messages.push(message);rememberPublished(this.store,message);room.updatedAt=message.time;
  ...
  delete(id:string){const room=this.room(required(id,'群聊 ID',80));this.stop(id);this.store.data.groups=this.store.data.groups.filter(item=>item!==room);this.store.data.groupDeliveries=this.store.data.groupDeliveries.filter(d=>d.groupId!==id);this.touch();}
  ...
  private async process(room:GroupRoom,worker:Worker){
    ...
    const emittedPin=room.messages.find(message=>message.kind==='reaction'&&message.runIds?.includes(run.id));if(emittedPin){for(const delivery of deliveries){delivery.status='replied';delivery.replyMessageId=emittedPin.id;}return;}
  ...
  invoke(botId:string,runId:string,name:string,args:Record<string,unknown>,signal:AbortSignal,options:HarnessRunOptions){
    ...
    if(name==='groups_list')return this.store.data.groups.filter(room=>this.members(room).some(member=>member.id===botId)).map(room=>({id:room.id,name:room.name,members:this.members(room).map(m=>({id:m.id,name:m.name})),preview:room.messages.at(-1)?.content.slice(0,300)}));
