...
export class GroupChats implements GroupGateway {
  ...
  private append(room:GroupRoom,sender:GroupSender,content:string,round?:GroupRound,mentions?:BotMention[],kind:import('../../src/group-types').GroupMessage['kind']=sender.kind==='system'?'system':'message',replyTo?:string,extra:Partial<Pick<import('../../src/group-types').GroupMessage,'reaction'|'runIds'|'event'|'attachments'|'scheduled'|'workItemId'>>={}){
  ...
  delete(id:string){const room=this.room(required(id,'群聊 ID',80));this.stop(id);this.store.data.workItems=this.store.data.workItems?.filter(item=>item.scope.kind!=='group'||item.scope.id!==id);if(this.store.data.conversationWorkspaces)delete this.store.data.conversationWorkspaces['group:'+id];this.store.data.groups=this.store.data.groups.filter(item=>item!==room);this.store.data.groupDeliveries=this.store.data.groupDeliveries.filter(d=>d.groupId!==id);this.touch();}
