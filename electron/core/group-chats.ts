...
function backups(value:unknown,members:Set<string>,coordinator:string){if(!Array.isArray(value)||value.some(id=>typeof id!=='string'))throw new Error('备用协调 Bot 无效');return [...new Set(value)].filter(id=>members.has(id)&&id!==coordinator).slice(0,2);}
export class GroupChats implements GroupGateway {
  ...
  private coordinator(room:GroupRoom){const members=this.members(room),valid=(id:string|undefined)=>id&&members.some(member=>member.id===id)?id:undefined;return valid(room.coordinatorBotId)||room.backupCoordinatorBotIds?.map(valid).find(Boolean)||members[0]?.id;}
  ...
  private newRound(room:GroupRoom,request:string,originKey?:string,ownerBotId?:string){const round:GroupRound={id:randomUUID(),groupId:room.id,request:request.slice(0,8000),ownerBotId:ownerBotId||this.coordinator(room),status:'active',createdAt:now(),botMessages:0,botCounts:{},decisions:0,createdGroups:0,originKey};this.store.data.groupRounds.push(round);room.activeRootId=round.id;return round;}
  private recipients(room:GroupRoom,message:import('../../src/group-types').GroupMessage,round:GroupRound,notify:string[]=[],delegatedResult=false){const members=this.members(room),memberIds=new Set(members.map(member=>member.id));if(delegatedResult)return [...new Set(notify)].filter(id=>memberIds.has(id)&&id!==message.sender.id);const mentioned=(message.mentions||[]).map(mention=>mention.id).filter(id=>memberIds.has(id)&&id!==message.sender.id),explicit=[...new Set([...mentioned,...notify])];if(explicit.length)return explicit;if((message.sender.kind==='user'||message.scheduled)&&individualGroupResponses(round.request))return members.filter(member=>member.id!==message.sender.id).map(member=>member.id);if(message.sender.kind==='user'||message.scheduled)return [round.ownerBotId||this.coordinator(room)].filter((id):id is string=>Boolean(id)&&id!==message.sender.id);return [];}
  private append(room:GroupRoom,sender:GroupSender,content:string,round?:GroupRound,mentions?:BotMention[],kind:import('../../src/group-types').GroupMessage['kind']=sender.kind==='system'?'system':'message',replyTo?:string,extra:Partial<Pick<import('../../src/group-types').GroupMessage,'reaction'|'runIds'|'event'|'attachments'|'scheduled'|'workItemId'|'reply'|'previewPrompt'|'delegation'>>&{notifyBotIds?:string[];delegatedByBotId?:string;delegatedResult?:boolean}={}){
    if(sender.kind==='user')sender={...sender,name:userDisplayName(this.store.data.userProfile)};
    const {notifyBotIds,delegatedByBotId,delegatedResult,...messageExtra}=extra;const message:import('../../src/group-types').GroupMessage={...(sender.kind==='user'?{workspaceDir:effectiveWorkspace(this.store,this.host,{kind:'group',id:room.id})}:{}),id:randomUUID(),seq:(room.messages.at(-1)?.seq||0)+1,groupId:room.id,sender,kind,content,time:now(),rootId:round?.id,mentions,replyTo,...messageExtra};room.messages.push(message);rememberPublished(this.store,message);room.updatedAt=message.time;
    if(round){const current=this.store.data.groupRounds.findIndex(r=>r.id===room.activeRootId);if(current<0||this.store.data.groupRounds.indexOf(round)>=current)room.activeRootId=round.id;for(const recipientId of ['user',...this.recipients(room,message,round,notifyBotIds,delegatedResult)].filter(id=>id!==sender.id))this.store.data.groupDeliveries.push({id:randomUUID(),groupId:room.id,messageId:message.id,recipientId,rootId:round.id,status:recipientId==='user'?'delivered':'queued',createdAt:message.time,...(delegatedByBotId&&recipientId!=='user'?{delegatedByBotId}:{})});if(sender.kind==='bot'){round.botMessages++;round.botCounts[sender.id]=(round.botCounts[sender.id]||0)+1;}this.updated(room,sender.id);}
    return message;
  }
  ...
  private createRoom(name:string,bots:Bot[],sender:GroupSender,round?:GroupRound,coordinatorBotId?:string,backupCoordinatorBotIds?:string[]){const ids=new Set(bots.map(bot=>bot.id)),coordinator=coordinatorBotId||bots[0]?.id;if(!coordinator||!ids.has(coordinator))throw Error('协调 Bot 必须是群成员');const room:GroupRoom={id:randomUUID(),name,members:bots.map(bot=>({...identity(bot),joinedAt:now()})),createdBy:sender,createdAt:now(),updatedAt:now(),messages:[],lastReadSeq:0,coordinatorBotId:coordinator,backupCoordinatorBotIds:backups(backupCoordinatorBotIds||[],ids,coordinator)};this.store.data.groups.push(room);this.lifecycle(room,`${sender.name} 创建了群聊`,{type:'created',actor:sender,joined:bots.map(identity),left:[]},round);return room;}
  create(input:{name:string;botIds:string[];coordinatorBotId?:string;backupCoordinatorBotIds?:string[]}){const name=required(input?.name,'群名称',80),bots=this.botIds(input?.botIds,2);const room=this.createRoom(name,bots,human,undefined,input.coordinatorBotId,input.backupCoordinatorBotIds);this.touch();return this.summary(room);}
  update(input:{id:string;name:string;botIds:string[];coordinatorBotId?:string;backupCoordinatorBotIds?:string[]}){this.changeMembers(input,human);}
  private changeMembers(input:{id:string;name:string;botIds:string[];coordinatorBotId?:string;backupCoordinatorBotIds?:string[]},actor:GroupSender,round?:GroupRound){
    ...
    const coordinator=input.coordinatorBotId===undefined?this.coordinator(room):input.coordinatorBotId;if(!coordinator||!ids.has(coordinator))throw Error('协调 Bot 必须是群成员');const backupIds=input.backupCoordinatorBotIds===undefined?(room.backupCoordinatorBotIds||[]):input.backupCoordinatorBotIds,backupCoordinators=backups(backupIds,ids,coordinator);if(room.coordinatorBotId!==coordinator){room.coordinatorBotId=coordinator;this.append(room,system,`协调 Bot 改为「${this.store.bot(coordinator).name}」`);}room.backupCoordinatorBotIds=backupCoordinators;
  ...
  send(input:{id:string;message:string;previewPrompt?:string;replyToMessageId?:string;mentions?:BotMention[];attachmentIds?:string[]}){if(input.previewPrompt!==undefined&&(typeof input.previewPrompt!=='string'||input.previewPrompt.length>12000))throw Error('无效预览意见');const command=workCommand(input?.message||'');if(command&&!command.objective)throw Error(`请在 /${command.kind} 后填写任务内容`);const room=this.room(required(input?.id,'群聊 ID',80)),attachments=this.attachments.forDraft({kind:'group',id:room.id},input.attachmentIds);if(typeof input.message!=='string')throw new Error('消息无效');required(input.message||attachmentSummary(attachments),'消息',32000);const reply=resolveGroupReply(room,input.replyToMessageId),mentions=this.mentions(room,input.message,input.mentions),round=this.newRound(room,input.message||`用户发送了 ${attachments.length} 个附件。`,undefined,mentions[0]?.id);this.append(room,human,input.message,round,mentions,'message',undefined,{attachments,...(input.previewPrompt!==undefined?{previewPrompt:input.previewPrompt}:{}),...(reply?{reply}:{})});this.touch();}
  ...
  private async process(room:GroupRoom,worker:Worker){
    ...
    await this.runner.run(bot.id,JSON.stringify({events:deliveries.map(d=>{const message=room.messages.find(message=>message.id===d.messageId)!;return {eventId:d.id,messageId:message.id,sender:message.sender,kind:message.kind,reply:message.reply,event:message.event,delegation:message.delegation,scheduled:message.scheduled,content:groupReplyContent(message.content,message.sender.kind==='bot'?message.sender.id:undefined).slice(0,1600),mentions:message.mentions,mentioned:message.mentions?.some(mention=>mention.id===bot.id)||false};}),recent}),{resumeRunId:retry?.id,workItemId:retry?.workItemId||workItem?.id,workspaceDir:retry?.workspaceDir||workItem?.workspaceDir||lastMessage?.workspaceDir||effectiveWorkspace(this.store,this.host,{kind:'group',id:room.id}),groupOrigin:{groupId:room.id,rootId:round.id,deliveryId:deliveries.at(-1)!.id},groupContext:context+groupContributionContext(room,round.id,bot.id,deliveries.map(delivery=>delivery.messageId))+GROUP_STATE_EVENT_PROMPT,groupTaskFrom:previousTask?.id,onStarted:id=>{worker.runId=id;for(const delivery of deliveries)delivery.runId=id;this.touch();}});
    ...
    const handedOff=room.messages.some(message=>message.rootId===round.id&&message.delegation?.mode==='handoff'&&message.delegation.fromBotId===bot.id);
    if(handedOff){for(const delivery of deliveries){delivery.status='ignored';delivery.reason='已移交当前任务负责人';}return;}
    ...
    round.repetitions=0;const notify=[...new Set(deliveries.map(delivery=>delivery.delegatedByBotId).filter((id):id is string=>Boolean(id)&&id!==bot.id))];const message=this.append(room,{kind:'bot',...identity(bot)},answer,round,finalMessage?.mentions,'message',trigger?.id,{attachments:finalMessage?.attachments,notifyBotIds:notify,delegatedResult:notify.length>0});run.groupReplyMessageId=message.id;message.runIds=this.store.data.runs.filter(item=>item.botId===bot.id&&item.groupOrigin?.groupId===room.id&&item.startedAt>=deliveries[0].createdAt).map(item=>item.id);for(const delivery of deliveries){delivery.status='replied';delivery.replyMessageId=message.id;}
  }
  ...
  invoke(botId:string,runId:string,name:string,args:Record<string,unknown>,signal:AbortSignal,options:HarnessRunOptions){
    ...
    const groupId=name==='group_wake'?options.groupOrigin?.groupId:args.groupId,room=this.room(required(groupId,'群聊 ID',80));this.member(room,botId);
    ...
    if(name==='group_wake'){
      if(round.ownerBotId&&round.ownerBotId!==botId)throw new Error('只有当前任务负责人可以委托群成员');
      if(!Array.isArray(args.botIds)||!args.botIds.length||args.botIds.length>4||args.botIds.some(id=>typeof id!=='string'))throw new Error('请选择 1–4 位群成员');const targets=[...new Set(args.botIds)];for(const target of targets){this.member(room,target);if(target===botId)throw new Error('不能委托自己');}
      if(args.mode!=='assist'&&args.mode!=='handoff')throw new Error('委托模式无效');const task=required(args.task,'子任务',4000),mode=args.mode,fingerprint=normalized(task);round.delegations||={};if(targets.some(target=>round.delegations![target]===fingerprint))return {woken:false,duplicate:true,message:'相同成员已收到相同子任务。'};
      for(const target of targets)round.delegations[target]=fingerprint;if(mode==='handoff'){if(targets.length!==1)throw new Error('移交主责时只能选择一位成员');round.ownerBotId=targets[0];}
      const mentions=targets.map(target=>{const member=identity(this.store.bot(target));return {...member,name:member.name,start:0,end:0};});const event=this.append(room,system,`${this.store.bot(botId).name} ${mode==='handoff'?'移交':'委托'}：${task}`,round,mentions,'system',undefined,{delegation:{fromBotId:botId,mode,task},delegatedByBotId:botId});this.touch();return {woken:true,mode,botIds:targets,messageId:event.id};
    }
