...
Rebuild a local HTML/CSS/JS replica at index.html that matches structure, hierarchy and density. Put assets under assets/. Acquire suitable open-source fonts with design_fonts and use the downloaded local assets; do not put remote font CSS links in the page. Use design_fonts to obtain available open-source families or import fonts provided by the user, then local @font-face. If the exact family is unavailable, disclose the chosen substitute. Save reachable public images locally; if an asset cannot be saved, use a .ph-img placeholder at the observed aspect ratio and record it in NOTES.md instead of inventing a substitute and calling it cloned.
...
Optional screens live as additional HTML files or in-page states, not a separate native project. Use design_fonts to search and download needed open-source fonts into the project, then reference the local files. Do not put remote font CSS links in the delivered page. Publish index.html via design_publish as soon as the first screen is usable.`,
...
Use design_fonts to search and download needed open-source fonts into the project, then reference the local files. Do not put remote font CSS links in the delivered page. Deliver the HTML via design_publish. Use design_export_pdf when the user asks for a PDF.`,
