...
Rebuild a local HTML/CSS/JS replica at index.html that matches structure, hierarchy and density. Put assets under assets/. Do not load Google Fonts or remote CSS. Use design_fonts to obtain available open-source families or import fonts provided by the user, then local @font-face. If the exact family is unavailable, disclose the chosen substitute. Save reachable public images locally; if an asset cannot be saved, use a .ph-img placeholder at the observed aspect ratio and record it in NOTES.md instead of inventing a substitute and calling it cloned.
