...
export class ContextEngine {
  ...
  private calibrationKey(botId:string){const model=this.storage.store.modelFor(botId);return `token-calibration-v2:${sourceHash([{role:'user',content:JSON.stringify([model.providerId||'',model.protocol||'chat',model.baseUrl,model.model])}])}`;}
  private calibration(botId:string){return readCalibration(this.storage.get(this.calibrationKey(botId))).factor;}
  ...
  async prepare(input:ContextInput){const botId=input.botId;
    ...
    const transcript=new ContextView(this.storage,botId,stateKey),meter=new ContextMeter(this.storage,botId,stateKey,structuredClone(this.storage.store.modelFor(botId)),input.tools);
    ...
    if(estimate.tokens>budget.input){const reduced=pruning.prune(original,view,view.length,true);view=reduced.messages;prunedCount+=reduced.pruned;request=build(head,view);estimate=estimateFor(request);}
    ...
    while((estimate.tokens>budget.trigger||input.force&&compactions===0)&&compactions<8){
      ...
      try{
        ...
        try{summary=parseContextSummary(result.content,budget.summary);this.storage.contextAttempt(input.botId,input.runId,result.content);}
          ...
          const repaired=await this.model.complete(repairRequest,[],input.signal,()=>{},{botId,runId:input.runId,cacheScope:stateKey,purpose:'compaction',maxOutputTokens:budget.output});if(run)run.modelCalls++;this.observe(input.botId,input.runId,'compaction_repair',repaired,repairEstimate,calibration);if(input.signal.aborted)throw input.signal.reason?.name==='AbortError'?new Error('任务已取消'):input.signal.reason;if(repaired.calls.length)throw new Error('摘要修复尝试调用工具');
        ...
        const nextOriginal=input.history.slice(through),newView=pruning.prune(nextOriginal,pruning.apply(nextOriginal),nextOriginal.length,true).messages,nextRequest=build(next,newView),nextEstimate=estimateFor(nextRequest);
        ...
        head=next;original=nextOriginal;view=newView;request=nextRequest;estimate=nextEstimate;compactions++;
        this.storage.store.message(input.botId,'event','已整理较早的工作记录，可随时回查原文。',{runId:input.runId});
      }catch(error){transcript.restore(viewCheckpoint);if(input.signal.aborted||/原始历史发生变化|上下文版本发生变化/.test((error as Error).message))throw error;lastIssue=(error as Error).message;this.cooldown.set(stateKey,{until:Date.now()+60000,modelKey:contextModelKey(this.storage.store.modelFor(botId))});break;}
    }
