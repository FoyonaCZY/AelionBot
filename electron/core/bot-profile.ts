...
export function updateBotProfile(store:Store,providers:ModelProviders,input:BotUpdateInput,beforeModelChange:(botId:string)=>void=()=>{}){
  ...
  const next={...store.data,bots:store.data.bots.map(item=>item.id===bot.id?{...item,name:input.name.trim(),role:input.role,model,...(palette?{color:palette.color,avatarStyle:palette.avatarStyle}:{})}:item)};
