...
export class Diagnostics {
  ...
  private async collect(){
    ...
    const report=clean({
      ...
      runs,modelUsage:state.modelUsage?.slice(-100).map(record=>({...pick(record,['id','runId','purpose','model','time','estimatedTokens','error']),requestCache:record.requestCache?pick(record.requestCache,['version','scopeFingerprint','optionsFingerprint','toolsFingerprint','inputFingerprint','inputBytes','inputMessages','compared','firstDifference','firstDifferentMessage','matchingPrefixMessages','matchingInputPrefixBytes','prefixScanTruncated','promptCacheKeySent','cacheKeyRejected']):undefined,usage:record.usage?pick(record.usage,['version','inputTokens','outputTokens','totalTokens','cachedTokens','cacheWriteTokens','reasoningTokens','latencyMs','attempts']):undefined})),
