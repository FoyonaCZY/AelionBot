...
export class Diagnostics {
  ...
  private async collect(){
    ...
    const report=clean({
      ...
      runs,modelUsage:state.modelUsage?.slice(-100).map(record=>({...pick(record,['id','runId','purpose','model','time','estimatedTokens','error','transportErrorCodes']),timing:record.timing?pick(record.timing,['preparationMs','responseMs','firstEventMs','firstTextMs','firstToolMs','lastEventMs','responseBytes','outputEvents','toolArgumentChars','completed','totalMs']):undefined,context:record.context?pick(record.context,['estimatedTokens','calibration','prunedOutputs','epoch','estimateSource','contextChanges','archivedImages']):undefined,requestCache:record.requestCache?pick(record.requestCache,['version','scopeFingerprint','optionsFingerprint','toolsFingerprint','inputFingerprint','inputBytes','inputMessages','compared','firstDifference','firstDifferentMessage','firstDifferentRole','sessionAffinitySent','toolCount','toolsAdded','toolsRemoved','toolsChanged','matchingPrefixMessages','matchingInputPrefixBytes','prefixScanTruncated','promptCacheKeySent','cacheKeyRejected','transport','incremental','sentInputItems','successfulPrefixMessages','strictContinuation','imageCount','toolHistoryRepairs']):undefined,usage:record.usage?pick(record.usage,['version','inputTokens','outputTokens','totalTokens','cachedTokens','cacheWriteTokens','reasoningTokens','latencyMs','attempts']):undefined})),
    ...
    const env=report.environment,activeModel=latestError?.model||report.runs.at(-1)?.model||report.model;
    const summary=[`AelionBot ${env.appVersion}`,`系统：${env.platform} ${env.arch} / ${env.osRelease}`,`Electron：${env.electron} · Node：${env.node}`,`模型：${activeModel.model||'未配置'} (${activeModel.protocol||'chat'}) · 上下文：${activeModel.contextTokens}`,`工作电脑：${report.vm.status} · 镜像：${report.vm.imageVersion}`,`最近任务：${report.runs.length} 项，其中 ${report.runs.filter((run:any)=>run.status==='failed'||run.status==='interrupted').length} 项失败或中断`,latestError?`最近错误：${latestError.error.slice(0,600)}`:report.vm.lastError?`电脑错误：${report.vm.lastError.slice(0,600)}`:'',`报告时间：${createdAt}`,`报告编号：${id}`].filter(Boolean).join('\n');
