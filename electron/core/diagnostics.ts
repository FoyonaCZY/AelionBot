...
export class Diagnostics {
  ...
  private async collect(){
    ...
    const report=clean({
      ...
      runs,modelUsage:state.modelUsage?.slice(-100).map(record=>({...pick(record,['id','runId','purpose','model','time','estimatedTokens','error']),usage:record.usage?pick(record.usage,['inputTokens','outputTokens','cachedTokens','reasoningTokens','latencyMs','attempts']):undefined})),
