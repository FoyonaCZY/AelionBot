...
export type GuestImage=typeof x64Image & {provisionedWorkstation?:string};
export function vmPlatform(platform:NodeJS.Platform=process.platform,arch:string=process.arch){
 ...
 const image:GuestImage=guestArch==='arm64'?armImage:x64Image;
 return {arch:guestArch,image,accelerator:platform==='darwin'?'hvf':platform==='win32'?'whpx':'tcg',executable:`qemu-system-${guestArch==='arm64'?'aarch64':'x86_64'}${platform==='win32'?'.exe':''}`,imageTool:`qemu-img${platform==='win32'?'.exe':''}`,machine:guestArch==='arm64'?'virt':'q35'};
}
