...
export class ModelProviders {
  ...
  private commit(patch:Partial<Store['data']>){
    ...
    atomicJson(this.store.file,next);this.store.data=next;this.changed();
  }
  ...
  save(input:ProviderInput){
    ...
    const connectionChanged=!previous||previous.baseUrl!==baseUrl||previous.encryptedKey!==encryptedKey;
  ...
  refresh(id:string):Promise<ModelProvider>{
    const provider=this.provider(id),revision=JSON.stringify([provider.baseUrl,provider.encryptedKey]);
    ...
    const pending=(async()=>{
      ...
      try{
        const key=this.keyFor(provider),response=await fetch(`${validateModelEndpoint(provider.baseUrl)}/models`,{headers:{Accept:'application/json',...(key?{Authorization:`Bearer ${key}`}:{})},redirect:'error',signal:AbortSignal.any([controller.signal,AbortSignal.timeout(15000)])});
        if(!response.ok){await response.body?.cancel();throw new Error(`获取模型列表失败 HTTP ${response.status}`);}
        const body=await jsonBody(response);if(!Array.isArray(body?.data))throw new Error('模型列表格式不兼容，需要 data 数组');
      ...
      if(controller.signal.aborted||JSON.stringify([current.baseUrl,current.encryptedKey])!==revision)return this.public(current);
