...
export class CognitiveStore {
  ...
  commitEpoch(input:{botId:string;runId:string;expectedRevision:number;from:number;through:number;summary:string;anchors:string[];sourceHash:string;stats:unknown}){
    this.db.exec('BEGIN IMMEDIATE');try{const head=this.head(input.botId);if(head.revision!==input.expectedRevision)throw new Error('压缩期间上下文版本发生变化');const id=randomUUID();this.db.prepare('INSERT INTO context_epochs VALUES(?,?,?,?,?,?,?,?,?,?)').run(id,input.botId,input.runId,input.from,input.through,input.summary,JSON.stringify(input.anchors),input.sourceHash,JSON.stringify(input.stats),new Date().toISOString());this.db.prepare('INSERT INTO context_heads VALUES(?,?,?,?,?) ON CONFLICT(bot_id) DO UPDATE SET revision=excluded.revision,through_seq=excluded.through_seq,summary=excluded.summary,anchors=excluded.anchors').run(input.botId,head.revision+1,input.through,input.summary,JSON.stringify(input.anchors));this.db.exec('COMMIT');return id;}catch(error){this.db.exec('ROLLBACK');throw error;}
  }
