...
export class Harness {
  ...
  async run(botId:string,input:string,options:HarnessRunOptions={}){
    ...
    if(this.host&&this.interactions){
      ...
      system.content+="\nYou may operate host commands and files when needed. Use host_execute, host_file_read, and host_file_write for host repositories, files, and existing gh/git sessions. The app applies the current Bot permission mode: ask requires a human decision; auto permits ordinary workspace reads/writes and saved command rules, then asks the default model to review other operations; full follows the human selection. Never assume authorization; wait for actual tool results. Reading discovered skills and discovery/resource/template reads on enabled MCP services are available as needed. Host MCP calls and scripts follow this Bot permission mode. Aelion memories and private skills are internal application state. Do not bundle unrelated actions to reduce confirmations or rewrite commands to evade permission checks. Stop after denial; do not switch tools to bypass it. Only humans can grant permissions. Never modify permission files, use scripts, MCP, or UI automation to grant or expand authorization, or click Aelion permission buttons. Host CLIs reuse the existing environment and login: run gh directly, do not run gh auth token, read passwords/private keys, or copy credentials into the VM. Report only actual execution locations and results.";
    }
    ...
    if(options.groupOrigin){turnContext.content+=`\nThe following is your own main-chat and task context, to understand and continue previously assigned work. Other group members have not necessarily seen it: ${groupMainContext(this.store,botId,6500,this.cognition?.storage.head(botId).summary)}. Use history_search/history_read for older records and share results, progress, and paths only as needed for handoffs.`;}
