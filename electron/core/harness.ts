...
export class Harness {
  ...
  async run(botId:string,input:string,options:HarnessRunOptions={}){
    ...
    try {
      ...
      try{const producedGroupWork=options.groupOrigin&&this.store.runMessages(run.id).some(message=>message.role==='tool'&&isGroupWorkTool(message.tool));if(run.toolCalls>0&&options.peerOrigin?.kind!=='peer_summary'&&(!options.groupOrigin||producedGroupWork)&&(!groupRuntime.updated||run.toolCalls>0))await this.collectArtifacts?.(botId,run.id);}catch(error){this.store.message(botId,'event',`工作文件列表暂未更新：${(error as Error).message}`,{runId:run.id});}
