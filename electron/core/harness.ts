  ...
  async run(botId:string,input:string,options:HarnessRunOptions={}){
    ...
    try {
      ...
      for(let iteration=0;;iteration++){
        ...
        let availableTools=baseTools.filter(t=>!hiddenHosted.has(t.function.name)&&(t.function.name!=='video_frames'||Boolean(this.video))&&(t.function.name!=='open_preview'||Boolean(this.previews))&&(t.function.name!=='view_image'||Boolean(this.host?.options.imagePreview))&&(!['request_user_input','user_input_wait'].includes(t.function.name)||Boolean(this.interactions))&&(work.forRun(run)?.status!=='planning'||PLANNING_TOOLS.has(t.function.name))&&(t.function.name!=='chat_pin'||!options.groupOrigin&&!options.peerOrigin)&&(!/^groups?_/.test(t.function.name)||this.groups)&&(!options.groupOrigin||!['history_search','history_read','memory','skill_save','skill_patch','skill_file_write','skill_manage','bot_delegate_task','delegation_receipt','bot_send_message','start_main_task','group_send_message'].includes(t.function.name)));
