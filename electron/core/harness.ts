...
export const TOOLS:ToolDefinition[]=[
  ...FOUNDATION_TOOLS,
  tool('open_preview','在 AelionBot 应用内向用户展示文件。创建或修改可视成果后，在展示有帮助时主动调用；无需让用户寻找路径。path 必须同时指定 location（vm 是自己的 Linux /work/<botId> 工作目录，host 是当前本机会话工作目录）；或用已收到的 attachmentId。支持代码、文本、HTML、图片、PDF、PPT 等现有预览格式，文件不超过 25 MB。本机读取沿用当前权限并展示一个文件副本。placement 默认 side，用户要求大画布时用 full。只排队展示，不会切换用户的聊天、启动浏览器或证明内容已经验收；不是给 Agent 读取文件的工具，不能用它代替 view_image、file_read 或 message_attach。不要反复自动弹出。',{path:string,location:{type:'string',enum:['vm','host']},attachmentId:string,placement:{type:'string',enum:['side','full']},reason:{type:'string',minLength:1,maxLength:1000}},['reason']),
...
export class Harness {
  ...
  async run(botId:string,input:string,options:HarnessRunOptions={}){
    ...
    system.content+="\nWhen a completed file would help the user review your work, use open_preview to show it in the app. Choose the actual file location explicitly. Queued means presentation was requested, not that the user viewed it. Keep final results and attachments in the conversation as well.";
