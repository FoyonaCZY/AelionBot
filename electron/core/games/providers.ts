...
export interface GameProviderConfig {
 ...
 backend?:'responses'|'chat-completions';contextTokens?:number;reasoningEffort?:string;maxOutputTokens?:number;
}
...
export function gameProviders(config:Omit<GameProviderConfig,'id'|'name'> & {additionalProviders?:GameProviderConfig[]}) {
 ...
 return {
  ...
  async decide(player:GamePlayer,context:GameView,request:GameRequest,signal:AbortSignal,options?:DecisionOptions){
   ...
   const body=chat?{model:p.model,messages:[{role:'system',content:identity+instructions},{role:'user',content:input}],max_completion_tokens:output,stream:false,...(p.model.startsWith('mimo-')?{response_format:{type:'json_object'}}:{}),...(p.model==='mimo-v2.6-flash'?{thinking:{type:'disabled'}}:{})}:{model:p.model,instructions,input,reasoning:{effort:p.reasoningEffort||'low'},max_output_tokens:output,store:false};
   ...
   const reasoningTokens=result.usage?.completion_tokens_details?.reasoning_tokens??result.usage?.output_tokens_details?.reasoning_tokens;
   if(chat?result.choices?.[0]?.finish_reason==='length':result.status==='incomplete'&&result.incomplete_details?.reason==='max_output_tokens')throw new GameModelError('format',p.name+' 输出被截断：达到生成上限（推理 token '+(reasoningTokens??'未知')+'）');
