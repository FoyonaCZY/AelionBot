...
export interface StoredProvider extends Omit<ModelProvider,'hasKey'> {encryptedKey?:string;hostedWebSearch?:boolean;hostedImageGeneration?:boolean;}
...
export class Store {
  ...
  modelFor(botId?:string,selectionOverride?:ModelSelection):ModelConfig {
    ...
    const provider=this.data.providers.find(provider=>provider.id===selection.providerId),catalog=provider?.models.find(model=>model.id===selection.model);
    return {...selection,contextTokens:catalog?.contextTokens??selection.contextTokens,supportsImages:selection.supportsImages??catalog?.supportsImages,protocol:provider?.protocol,responsesTransport:provider?.responsesTransport,hostedWebSearch:provider?.protocol==='responses'?catalog?.hostedWebSearch:undefined,hostedImageGeneration:provider?.protocol==='responses'?catalog?.hostedImageGeneration:undefined,temperature:provider?.temperature,reasoningEffort:botId?this.bot(botId).reasoningEffort||catalog?.reasoningEffort:selection.reasoningEffort||catalog?.reasoningEffort,thinkingBudget:catalog?.thinkingBudget??provider?.thinkingBudget,fallbackModel:provider?.fallbackModel,baseUrl:provider?.baseUrl||'',hasKey:Boolean(provider?.encryptedKey),providerName:provider?.name,...(!provider?{issue:'所选 Provider 不存在，请重新选择模型'}:{})};
  }
