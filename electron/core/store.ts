...
export class Store {
  ...
  modelFor(botId?:string,selectionOverride?:ModelSelection):ModelConfig {
    ...
    return {...selection,contextTokens:catalog?.contextTokens??selection.contextTokens,supportsImages:catalog?.supportsImages,protocol:provider?.protocol,responsesTransport:provider?.responsesTransport,hostedWebSearch:provider?.protocol==='responses'?provider.hostedWebSearch:undefined,hostedImageGeneration:provider?.protocol==='responses'?provider.hostedImageGeneration:undefined,imageProtocol:provider?.imageProtocol,imageAspect:catalog?.imageAspect??provider?.imageAspect,imageQuality:catalog?.imageQuality??provider?.imageQuality,temperature:provider?.temperature,reasoningEffort:botId?this.bot(botId).reasoningEffort||catalog?.reasoningEffort:selection.reasoningEffort||catalog?.reasoningEffort,thinkingBudget:catalog?.thinkingBudget??provider?.thinkingBudget,fallbackModel:provider?.fallbackModel,baseUrl:provider?.baseUrl||'',hasKey:Boolean(provider?.encryptedKey),providerName:provider?.name,...(!provider?{issue:'所选 Provider 不存在，请重新选择模型'}:{})};
  }
