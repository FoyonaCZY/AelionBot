...
async function bytes(vm:VmController,botId:string,path:string,signal:AbortSignal){
...
export async function readVmFile(vm:VmController,botId:string,path:string,args:Record<string,unknown>,signal:AbortSignal,redact:(text:string)=>string=text=>text){
  ...
  const source=await bytes(vm,botId,path,signal),decoded=decodeText(source.bytes),safe=redact(decoded.text),{content,...page}=textPage(safe,range);
...
export async function patchVmFile(vm:VmController,checkpoints:FileCheckpoints,botId:string,runId:string,path:string,args:Record<string,unknown>,signal:AbortSignal,redact:(text:string)=>string=text=>text){
  ...
  const edit={oldText:args.oldText,newText:args.newText,expectedSha256:expected,replaceAll:args.replaceAll},source=await bytes(vm,botId,path,signal);
