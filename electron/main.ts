...
import {LayaShadow} from './core/laya-shadow';
import {LayaFeature} from './core/laya-feature';
...
async function initialize(){
  ...
  groupChats=new GroupChats(store,{isRunning:id=>updatePreparing||harness.isRunning(id)||Boolean(chatPins?.hasPending(id)),run:(id,input,options)=>{greetings?.cancel(id);return harness.run(id,input,options);},cancel:id=>harness.cancel(id),refresh:id=>harness.refreshGroup(id)},changed,attachments,host,laya);
  ...
  const shutdown=new Shutdown({
    stop:()=>{vm.beginShutdown();if(timer)clearInterval(timer);for(const close of [()=>appUpdates?.dispose(),()=>scheduler?.dispose(),()=>greetings?.dispose(),()=>{for(const bot of store.data.bots)harness.cancel(bot.id);},()=>providers.dispose(),()=>games?.dispose(),()=>layaFeature?.dispose(),()=>laya?.dispose(),()=>model?.dispose(),()=>harness.disposeTools(),()=>videoInspector?.dispose(),()=>webPreview?.close(),()=>chatPins?.dispose(),()=>peerChats?.dispose(),()=>groupChats?.dispose(),()=>interactions.dispose(),()=>host.dispose()])try{close();}catch(error){diagnostics?.record('app.shutdown-error',error);}},
