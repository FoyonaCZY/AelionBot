...
export class WebPreviewBrowser {
 ...
 constructor(private window:BrowserWindow,private vm:VmController,private artifacts:ArtifactService,private attachments:Attachments){window.once('closed',()=>this.close());window.webContents.on('render-process-gone',()=>this.close());window.webContents.on('did-start-loading',()=>this.close());}
 private emit(active:Active){if(this.active!==active)return;active.view.setVisible(active.visible&&!active.state.error);if(!this.window.isDestroyed())this.window.webContents.send('web-preview:event',active.state);}
