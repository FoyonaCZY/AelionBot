...
export function ModelSelectionFields({providers,value,onChange,defaultModel,inheritDefault=false,disabled=false,reasoningValue,onReasoningChange,showInheritedReasoning=true}:{showInheritedReasoning?:boolean;providers:ModelProvider[];value:ModelSelection|null;onChange:(value:ModelSelection|null)=>void;defaultModel?:ModelSelection;inheritDefault?:boolean;disabled?:boolean;reasoningValue?:string;onReasoningChange?:(value:string)=>void}){
  ...
  return <div className="model-selection-fields">
    ...
    {(!inheritDefault||value!==null)&&<div className="settings-card">
      ...
      <label className="settings-row"><span>{t('模型')}</span><EditableSelect label={t('使用的模型')} value={value?.model||''} disabled={!provider||disabled} onChange={model=>{const entry=provider?.models.find(item=>item.id===model);update({model,supportsImages:entry?.supportsImages,contextTokens:entry?.contextTokens||value?.contextTokens||32000,reasoningEffort:entry?.reasoningEffort});}} options={ids} placeholder={t('选择或输入模型名称')}/></label>
      {(()=>{const entry=provider?.models.find(item=>item.id===value?.model);return entry?.contextTokens?<p className="model-catalog-note">{t('上下文 {tokens} Token',{tokens:entry.contextTokens.toLocaleString()})}{entry.reasoningEffort?` · ${entry.reasoningEffort}`:''}{entry.hostedWebSearch?` · ${t('服务端搜索')}`:''}{entry.hostedImageGeneration?` · ${t('服务端生图')}`:''}</p>:<label className="settings-row settings-number"><span>{t('上下文容量')}</span><ContextCapacityInput value={value?.contextTokens??32000} disabled={!value||disabled} onChange={contextTokens=>update({contextTokens})}/></label>;})()}
