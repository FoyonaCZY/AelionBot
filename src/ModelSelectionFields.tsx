...
export function ModelSelectionFields({providers,value,onChange,defaultModel,inheritDefault=false,disabled=false,reasoningValue,onReasoningChange,showInheritedReasoning=true,noneLabel}:{showInheritedReasoning?:boolean;providers:ModelProvider[];value:ModelSelection|null;onChange:(value:ModelSelection|null)=>void;defaultModel?:ModelSelection;inheritDefault?:boolean;disabled?:boolean;reasoningValue?:string;onReasoningChange?:(value:string)=>void;noneLabel?:string}){
  const {t}=useI18n();
  ...
  return <div className="model-selection-fields">
    ...
    {(!inheritDefault||value!==null)&&<div className="settings-card">
      ...
      <label className="settings-row"><span>{t('模型')}</span><EditableSelect label={t('使用的模型')} value={value?.model||''} disabled={!provider||disabled} onChange={model=>{const entry=provider?.models.find(item=>item.id===model);update({model,contextTokens:entry?.contextTokens||value?.contextTokens||32000,reasoningEffort:entry?.reasoningEffort});}} options={ids} placeholder={t('选择或输入模型名称')}/></label>
      ...
      {!(provider?.models.some(item=>item.id===value?.model&&item.reasoningEffort))&&reason}
    </div>}
