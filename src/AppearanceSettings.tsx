...
export function AppearanceSettings({settings,change,saving,error}:{settings:Values;change:(patch:Partial<Values>)=>void;saving:boolean;error:string}){
  ...
  return <div className="appearance-settings">
    <section className="appearance-section"><h3>{t('语言')}</h3><Row title={t('界面语言')}><Select aria-label={t('选择界面语言')} value={language} onChange={event=>setLanguage(event.target.value as typeof language)}>{languageOptions.map(option=><option value={option.value} key={option.value}>{option.label}</option>)}</Select></Row></section>
    ...
    <div className="appearance-sample" aria-label={t('外观实时预览')}><span>{t('实时预览')}</span><h3>{t('字体预览')}</h3><p>{t('正文示例：今天的工作记录。')} The quick brown fox jumps over the lazy dog.</p><code>const progress = 100;</code></div>
    <section className="appearance-section"><h3>{t('字体')}</h3>
      ...
      <Row title={t('正文字重')}><Select aria-label={t('正文字重')} value={String(settings.weight)} onChange={e=>change({weight:Number(e.target.value)})}><option value="300">{t('纤细 · 300')}</option><option value="400">{t('常规 · 400')}</option><option value="500">{t('中等 · 500')}</option><option value="600">{t('较粗 · 600')}</option></Select></Row>
    </section>
    <section className="appearance-section"><h3>{t('大小与间距')}</h3>
      <Row title={t('界面文字')}>{number('textScale','界面文字大小',85,130,5,'%')}</Row>
      ...
      <Row title={t('显示比例')}><Select aria-label={t('显示比例')} value={String(settings.zoom)} onChange={e=>change({zoom:Number(e.target.value)})}>{[...new Set([50,75,90,100,110,125,150,175,200,settings.zoom])].sort((a,b)=>a-b).map(value=><option key={value} value={String(value)}>{value}%{value===100?` · ${t('默认')}`:''}</option>)}</Select></Row>
    </section>
