 ...
 return <main className="plugins-page" aria-labelledby="plugins-heading">
  ...
  {selected?<PluginDetails key={selected.kind+':'+selected.id} skill={selectedSkill} server={selectedServer} owner={state.bots.find(bot=>bot.id===selectedSkill?.botId)} busy={busy} act={act} onBack={()=>setSelected(undefined)}/>:<>
   ...
   <div className="plugins-list">
    ...
    {count===0&&<div className="plugins-empty"><span><Icon name={search?'search':'plugin'} size={24}/></span><h2>{search?label('没有找到相关插件','No matching plugins','找不到相關外掛'):label('暂无插件','No plugins','尚無外掛')}</h2>{search&&<button className="text-button" onClick={()=>setQuery('')}>{label('清除搜索','Clear search','清除搜尋')}</button>}</div>}
   </div>
