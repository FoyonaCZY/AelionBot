...
export function usePreviewEdits(externalDirty=false){
  ...
  useEffect(()=>{void window.aelion.setPreviewDirty?.(dirty||externalDirty).catch(()=>{});},[dirty,externalDirty]);
  useEffect(()=>()=>{void window.aelion.setPreviewDirty?.(false).catch(()=>{});},[]);
  useEffect(()=>{if(!dirty&&!saving&&!externalDirty)return;const before=(e:BeforeUnloadEvent)=>{e.preventDefault();e.returnValue='';};window.addEventListener('beforeunload',before);return()=>window.removeEventListener('beforeunload',before);},[dirty,saving,externalDirty]);
  ...
  return {drafts,dirty,saving,saved:(item:PreviewItem,value:EditableText)=>update(item.id,{...value,item,baseline:value.content,editing:false,loading:false,saving:false,error:''}),pending,guardError,notice,edit,save,request,saveAndContinue,discardAndContinue,cancel:()=>setPending(undefined),drop:(id:string)=>update(id,undefined),change:(id:string,content:string)=>{const d=ref.current[id];if(d&&!d.saving)update(id,{...d,content,error:''});}};
}
