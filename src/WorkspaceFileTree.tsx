...
function Branch({botId,path,activePath,onOpen,revision,depth=0}:{botId:string;path:string;activePath?:string;onOpen:(file:WorkspaceFileEntry)=>void;revision:number;depth?:number}){
  ...
  if(error)return <div className="wft-state" role="alert">{error}<button onClick={()=>setRetry(v=>v+1)}>重试</button></div>;
  if(!data)return <div className="wft-state" role="status">正在读取…</div>;
  return <ul className="wft-list">{data.entries.map(entry=><Entry key={entry.path} entry={entry} botId={botId} activePath={activePath} onOpen={onOpen} revision={revision} depth={depth}/>)}{!data.entries.length&&<li className="wft-state">空文件夹</li>}{data.truncated&&<li className="wft-state">此目录仅显示前 500 项</li>}</ul>;
}
...
export function WorkspaceFileTree({botId,activePath,onOpen}:{botId:string;activePath?:string;onOpen:(file:WorkspaceFileEntry)=>void}){
  ...
  return <div className="workspace-file-tree"><header><span>工作目录</span><button type="button" aria-label="刷新目录" title="刷新目录" onClick={()=>setRevision(v=>v+1)}><Icon name="restart" size={15}/></button></header><nav aria-label="工作目录文件结构"><Branch key={botId} botId={botId} path="" activePath={activePath} onOpen={onOpen} revision={revision}/></nav></div>;
}
