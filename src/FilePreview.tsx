...
import {previewOpenTarget,type PreviewOpenAction} from './preview-open';
...
import {canvasDesignSource,type CanvasExportFormat} from './canvas-export';
...
export function FilePreview({items:initialItems,initialIndex=0,initialExpanded=false,feedbackScope,onClose,registerGuard,onSessionChange,initialAnnotations}:{initialAnnotations?:Record<string,PreviewAnnotation[]>;items:PreviewItem[];feedbackScope?:AttachmentScope;initialIndex?:number;initialExpanded?:boolean;onClose:()=>void;registerGuard?:RegisterPreviewGuard;onSessionChange?:(value:{annotationCache?:Record<string,PreviewAnnotation[]>;items:PreviewItem[];index:number;expanded:boolean})=>void}){
  ...
  const [exportState,setExportState]=useState<CanvasExportState>(),exportInFlight=useRef(false);
  ...
  const exportCanvas=async(format:CanvasExportFormat|'original')=>{
    if(exportInFlight.current)return;exportInFlight.current=true;
    const job={format,name:item.name,itemId:item.id};setBusy(true);setNotice('');setExportState({...job,phase:'running'});
    try{
      if(format==='original'){if(!item.save)throw Error(l('当前文件无法导出','This file cannot be exported'));const result=await item.save();setExportState({...job,phase:result===null?'cancelled':'success',detail:typeof result==='string'?result:undefined});return;}
      const source=canvasDesignSource(item);if(!source)throw Error(l('此格式需要 HTML 设计作品','This format requires an HTML design'));
      const bounds=panel.current?.querySelector('.web-preview-slot')?.getBoundingClientRect()||panel.current?.querySelector('.fp-content')?.getBoundingClientRect();
      const viewport=web?.state.viewport?{width:web.state.viewport.width,height:web.state.viewport.height}:bounds?{width:bounds.width,height:bounds.height}:undefined;
      const result=await window.aelion.exportDesignFile({...source,format,viewport});
      setExportState(result?{...job,phase:'success',detail:result.path+(result.warnings.length?' · '+result.warnings.join('；'):'')}:{...job,phase:'cancelled'});
    }catch(error){setExportState({...job,phase:'error',detail:previewErrorText(error)});}finally{exportInFlight.current=false;setBusy(false);}
  };
  const openPreviewFile=async(action:PreviewOpenAction|'save')=>{if(action==='save'){await act(()=>item.save!(),t('已保存'));return;}const target=previewOpenTarget(item);if(target)await act(()=>window.aelion.openPreviewFile({target,action}));};
  const select=(next:number)=>{setIndex(next);setNotice('');if(!exportInFlight.current)setExportState(undefined);setRetry(0);};
  return createPortal(<PreviewRuntimeContext.Provider value={{request,initialAnnotations:annotationCache.current['web:'+item.id],rememberAnnotations:value=>rememberAnnotations({['web:'+item.id]:value}),mode,tool,setMode,setTool,web,setWeb,annotations,setFileAnnotations,sendEdits:async changes=>{if(!feedbackSubmit.current)throw Error('请从会话中打开预览后发送修改');await feedbackSubmit.current({text:t('请将预览中的修改应用到项目源码，保留原有功能并验证结果。'),edits:changes});return true;},saved:value=>edits.saved(item,value)}}><PreviewLayoutContext.Provider value={true}><div className={`fp-layer is-immersive ${wantsStudio?'is-studio':modal?'is-expanded':'is-docked'} ${feedbackScope?'has-feedback':''} ${exportState?'has-export-status':''}`} data-device={item.deviceFrame||undefined} onMouseDown={event=>{if(event.target===event.currentTarget&&modal&&!feedbackLock.current)request(onClose);}}><section ref={panel} className="fp-panel" data-layout={wantsStudio?'studio':modal?'expanded':'docked'} role={modal?'dialog':'region'} aria-modal={modal||undefined} aria-label={t('预览 {name}',{name:item.name})} tabIndex={-1} onKeyDown={event=>{
    ...
    {exportState&&<CanvasExportStatus state={exportState} onDismiss={()=>setExportState(undefined)} onRetry={exportState.itemId===item.id?()=>request(()=>void exportCanvas(exportState.format)):undefined}/>}
