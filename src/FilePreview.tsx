...
export function FilePreview({items:initialItems,initialIndex=0,initialExpanded=false,feedbackScope,onClose,registerGuard}:{items:PreviewItem[];feedbackScope?:AttachmentScope;initialIndex?:number;initialExpanded?:boolean;onClose:()=>void;registerGuard?:RegisterPreviewGuard}){
  ...
  return createPortal(<PreviewLayoutContext.Provider value={true}><div className={`fp-layer is-immersive ${modal?'is-expanded':'is-docked'} ${modal&&feedbackScope?'has-feedback':''}`} onMouseDown={event=>{if(event.target===event.currentTarget&&modal&&!feedbackLock.current)edits.request(onClose);}}><section ref={panel} className="fp-panel" data-layout={modal?'expanded':'docked'} role={modal?'dialog':'region'} aria-modal={modal||undefined} aria-label={t('预览 {name}',{name:item.name})} tabIndex={-1} onKeyDown={event=>{
    ...
    {feedbackScope&&<div style={{display:modal?undefined:'none'}} inert={Boolean(edits.pending)||!modal}><PreviewFeedback scope={feedbackScope} item={item} panel={panel} unsaved={dirty} onBusy={value=>{feedbackLock.current=value;setFeedbackSending(value);if(!value&&afterFeedback.current){const next=afterFeedback.current;afterFeedback.current=undefined;edits.request(next);}}}/></div>}
