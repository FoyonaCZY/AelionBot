...
function PreviewContent({item,retry,override}:{item:PreviewItem;retry:number;override?:ArtifactPreview}){
  ...
  if(native&&value.kind==='html'){
    ...
    return <WebPreview source={{kind:'document',format:'html',name:item.name,workspace:item.workspace,attachmentId,...(override||!item.workspace&&!attachmentId?{content:value.content}:{} )}} onSource={()=>setShowSource(true)}/>;
  }
  if(native&&value.kind==='pdf'&&kind!=='演示文稿')return <WebPreview source={{kind:'document',format:'pdf',name:item.name,dataUrl:value.dataUrl}}/>;
...
export function FilePreview({items:initialItems,initialIndex=0,initialExpanded=false,feedbackScope,onClose,registerGuard}:{items:PreviewItem[];feedbackScope?:AttachmentScope;initialIndex?:number;initialExpanded?:boolean;onClose:()=>void;registerGuard?:RegisterPreviewGuard}){
  const {t}=useI18n();
  ...
  useEffect(()=>registerGuard?.(proceed=>{if(feedbackLock.current)afterFeedback.current=proceed;else edits.request(proceed);}),[registerGuard,edits.request]);
  ...
  return createPortal(<PreviewLayoutContext.Provider value={true}><div className={`fp-layer is-immersive ${modal?'is-expanded':'is-docked'} ${feedbackScope?'has-feedback':''}`} onMouseDown={event=>{if(event.target===event.currentTarget&&modal&&!feedbackLock.current)edits.request(onClose);}}><section ref={panel} className="fp-panel" data-layout={modal?'expanded':'docked'} role={modal?'dialog':'region'} aria-modal={modal||undefined} aria-label={t('预览 {name}',{name:item.name})} tabIndex={-1} onKeyDown={event=>{
    ...
    {feedbackScope&&<div inert={Boolean(edits.pending)}><PreviewFeedback scope={feedbackScope} item={item} panel={panel} unsaved={dirty} onBusy={value=>{feedbackLock.current=value;setFeedbackSending(value);if(!value&&afterFeedback.current){const next=afterFeedback.current;afterFeedback.current=undefined;edits.request(next);}}}/></div>}
