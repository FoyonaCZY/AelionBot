...
function ImagePreview({src,name}:{src:string;name:string}){
  ...
  return <><PreviewToolbar><div className="fp-tools"><button onClick={()=>change(-.25)} disabled={scale<=.1} title={t('缩小')} aria-label={t('缩小图片')}><PreviewIcon name="minus"/></button><output>{Math.round(scale*100)}%</output><button onClick={()=>change(.25)} disabled={scale>=4} title={t('放大')} aria-label={t('放大图片')}><PreviewIcon name="plus"/></button><button aria-pressed={zoom===1} onClick={()=>setZoom(1)} title={t('原始尺寸')} aria-label={t('原始尺寸')}>1:1</button></div></PreviewToolbar>
...
export function FilePreview({items:initialItems,initialIndex=0,initialExpanded=false,feedbackScope,onClose,registerGuard,onSessionChange,initialAnnotations}:{initialAnnotations?:Record<string,PreviewAnnotation[]>;items:PreviewItem[];feedbackScope?:AttachmentScope;initialIndex?:number;initialExpanded?:boolean;onClose:()=>void;registerGuard?:RegisterPreviewGuard;onSessionChange?:(value:{annotationCache?:Record<string,PreviewAnnotation[]>;items:PreviewItem[];index:number;expanded:boolean})=>void}){
  ...
  useEffect(()=>{setMode('browse');setFileAnnotations([]);},[index]);
  ...
  useEffect(()=>{onSessionChange?.({items,index,expanded:modal});workbench?.update({scope:feedbackScope,itemId:item.id,name:item.name,docked:!modal,annotations});},[items,index,modal,feedbackScope?.kind,feedbackScope?.id,onSessionChange,workbench?.update,annotations]);
  ...
  useLayoutEffect(()=>{if(!modal&&!edits.pending&&!domPending)return;const root=document.getElementById('root');if(!root)return;const wasInert=root.inert;root.inert=true;return()=>{root.inert=wasInert;};},[modal,Boolean(edits.pending),Boolean(domPending)]);
  useEffect(()=>{let live=true;const key=(event:KeyboardEvent)=>{
    ...
    if(event.key==='Escape'){if(domPending){event.preventDefault();if(!domSaving&&!webRef.current?.busy)setDomPending(undefined);return;}if(document.querySelector('.fp-file-select:open'))return;event.preventDefault();event.stopImmediatePropagation();if(edits.pending){if(!edits.saving)edits.cancel();}else if(expanded&&wide)setExpanded(false);else request(onClose);}
  };window.addEventListener('keydown',key,true);return()=>{live=false;window.removeEventListener('keydown',key,true);};},[modal,expanded,wide,onClose,edits.pending,edits.saving,draft,domPending,domSaving,item.id]);
  ...
  return createPortal(<PreviewRuntimeContext.Provider value={{request,initialAnnotations:annotationCache.current['web:'+item.id],rememberAnnotations:value=>rememberAnnotations({['web:'+item.id]:value}),mode,tool,setMode,setTool,web,setWeb,annotations,setFileAnnotations,sendEdits:async changes=>{if(!feedbackSubmit.current)throw Error('请从会话中打开预览后发送修改');await feedbackSubmit.current({text:t('请将预览中的修改应用到项目源码，保留原有功能并验证结果。'),edits:changes});return true;},saved:value=>edits.saved(item,value)}}><PreviewLayoutContext.Provider value={true}><div className={`fp-layer is-immersive ${modal?'is-expanded':'is-docked'} ${modal&&feedbackScope?'has-feedback':''}`} onMouseDown={event=>{if(event.target===event.currentTarget&&modal&&!feedbackLock.current)request(onClose);}}><section ref={panel} className="fp-panel" data-layout={modal?'expanded':'docked'} role={modal?'dialog':'region'} aria-modal={modal||undefined} aria-label={t('预览 {name}',{name:item.name})} tabIndex={-1} onKeyDown={event=>{
    ...
    <div className="fp-workbench-tools" inert={Boolean(edits.pending)||Boolean(domPending)||feedbackSending}>
      ...
      {web&&<button onClick={()=>{setMode('edit');web.inspect();}}><PreviewIcon name="pages"/>{t('结构')}</button>}
      <span className="fp-tools-space"/>
      {(['rect','element','arrow','pen','text'] as AnnotationTool[]).filter(type=>type!=='element'||web).map(type=><button key={type} aria-pressed={mode==='annotate'&&tool===type} title={t(({rect:'框选',element:'元素批注',arrow:'箭头',pen:'画笔',text:'文字标注'})[type])} aria-label={t(({rect:'框选',element:'元素批注',arrow:'箭头',pen:'画笔',text:'文字标注'})[type])} onClick={()=>{setMode('annotate');setTool(type);}}><PreviewIcon name={({rect:'rect',element:'comment',arrow:'arrow',pen:'edit',text:'text'})[type]}/></button>)}
      {annotations.length>0&&<><input className="fp-annotation-note" aria-label={t('标注说明')} placeholder={t('标注说明')} value={annotations.at(-1)?.text||''} maxLength={1000} onChange={event=>{const value=annotations.map((a,i)=>i===annotations.length-1?{...a,text:event.target.value}:a);if(web)void web.command({type:'annotations',annotations:value}).catch(e=>setNotice(e.message));else fileAnnotationSetter.current(value);}}/><button aria-label={t('撤销标注')} onClick={()=>{const value=annotations.slice(0,-1);if(web)void web.command({type:'annotations',annotations:value});else fileAnnotationSetter.current(value);}}><PreviewIcon name="minus"/></button></>}
      {web&&mode==='edit'&&<><button disabled={web.busy||!web.state.canUndo} title={t('撤销')+' (Ctrl+Z)'} aria-label={t('撤销')} onClick={()=>void web.command({type:'undo'}).catch(e=>setNotice(e.message))}><PreviewIcon name="left"/></button><button disabled={web.busy||!web.state.canRedo} title={t('重做')+' (Ctrl+Shift+Z)'} aria-label={t('重做')} onClick={()=>void web.command({type:'redo'}).catch(e=>setNotice(e.message))}><PreviewIcon name="right"/></button><button className="fp-save-dom" title={web.primaryLabel+' (Ctrl+S)'} disabled={!web.state.dirty||web.busy} onClick={()=>void web.save()}>{web.primaryLabel}</button></>}
    </div>
    <div className="fp-workspace" inert={Boolean(edits.pending)||Boolean(domPending)||feedbackSending}>{directoryOpen&&<aside className="fp-directory" aria-label={t('文件目录')}>{directoryBotId?<WorkspaceFileTree botId={directoryBotId} activePath={item.workspace?.path||''} onOpen={file=>{const next=workspacePreviewItem(directoryBotId,file),existing=items.findIndex(value=>value.id===next.id);if(existing>=0)select(existing);else{setItems([...items,next]);select(items.length);}if(!matchMedia('(min-width:700px)').matches)setDirectoryOpen(false);}}/>:<><div className="wft-state">{t('本次文件')}</div>{items.map((file,i)=><button key={file.id} className="fp-session-file" aria-current={i===index?'page':undefined} onClick={()=>select(i)}>{file.name}</button>)}</>}</aside>}<div className={`fp-content ${draft?'has-editor-session':''}`}>
      ...
      <FileAnnotationLayer initialCache={annotationCache.current} onCacheChange={rememberAnnotations} itemId={item.id} revision={String(retry)+':'+sourceRevision.current} mode={mode} tool={tool} onChange={setFileAnnotations} register={registerFileAnnotations}/>
  ...
  </section></div></PreviewLayoutContext.Provider></PreviewRuntimeContext.Provider>,document.body);
}
