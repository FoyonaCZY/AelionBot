...
function PreviewContent({item,retry}:{item:PreviewItem;retry:number}){
  ...
  return <><DocumentPreview value={value} name={item.name}/>{value.truncated&&<div className="fp-truncated" role="status">预览内容已截断，可保存原文件查看全部内容。</div>}</>;
}
export function FilePreview({items:initialItems,initialIndex=0,onClose}:{items:PreviewItem[];initialIndex?:number;onClose:()=>void}){
  const [items,setItems]=useState(initialItems),[directoryOpen,setDirectoryOpen]=useState(true);
  ...
  useEffect(()=>{const key=(event:KeyboardEvent)=>{if(event.key==='Escape'&&(modal||panel.current?.contains(document.activeElement))){if(document.querySelector('.fp-file-select:open'))return;event.preventDefault();event.stopImmediatePropagation();if(expanded&&wide)setExpanded(false);else onClose();}};window.addEventListener('keydown',key,true);return()=>window.removeEventListener('keydown',key,true);},[modal,expanded,wide,onClose]);
