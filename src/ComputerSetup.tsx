...
export function ComputerStatus({vm,onOpen,minimal=false,bot}:{vm:VmState;onOpen:()=>void;minimal?:boolean;bot?:Pick<Bot,'id'|'name'|'color'|'avatarStyle'>}){
  ...
  if(minimal){
    const title=t(view.kind==='stopped'?'电脑已关闭':view.kind==='invite'?'准备一个工作桌面':view.kind==='download'?'正在准备系统':view.kind==='starting'?'正在启动':view.kind==='stopping'?'正在关闭':view.kind==='installing'?'正在准备应用':view.kind==='restart'?'需要重启':view.kind==='error'?'准备未完成':view.kind==='incomplete'?'继续准备桌面':view.kind==='upgrade'?'更新工作环境':'桌面已就绪');
    const label=t(view.working?'查看进度':view.kind==='stopped'?'启动电脑':view.kind==='invite'?'开始设置':view.kind==='error'?'重试':view.kind==='restart'?'重启电脑':view.kind==='upgrade'?'更新桌面':'继续设置');
  ...
  return <section className="computer-setup-card" aria-label={t('工作电脑状态')}>
    ...
    <p>{view.kind==='stopped'&&vm.appsReady?t('启动后即可使用工作桌面。'):view.kind==='upgrade'?t('文件还在。更新后可以使用压缩包、表格脚本和演示文稿。'):view.kind==='restart'?t('应用已安装，请重启工作电脑继续。'):view.kind==='error'?t('查看原因后可继续重试。'):view.working?vm.detail:t('让 Bot 使用浏览器、文件和办公软件。')}</p>
    {view.working&&<SetupProgress vm={vm}/>}
    {view.kind==='upgrade'?<span className="setup-estimate-small">{t('通常几分钟，需要联网')}</span>:!(view.kind==='stopped'&&vm.appsReady)&&<span className="setup-estimate-small">{t('首次准备约 5～15 分钟')}</span>}
...
export function ComputerSetup({vm,disabled,bot,onClose,onReady,onNotify}:{vm:VmState;disabled:boolean;bot?:Pick<Bot,'id'|'name'|'color'|'avatarStyle'>;onClose:()=>void;onReady:()=>void;onNotify:(message:string)=>void}){
  ...
  const busy=view.working||pending,firstTime=view.kind==='invite';
  const title=t(error?'准备暂时停住了':pending?'正在唤醒桌面':({invite:'给伙伴一个桌面',stopped:'让伙伴开始工作',download:'正在准备新桌面',starting:'桌面正在醒来',stopping:'让桌面休息一下',installing:'把工具准备好',restart:'就差最后一步',error:'准备暂时停住了',incomplete:'接着准备桌面',upgrade:'给桌面补上新工具',ready:'一切就绪'}[view.kind]));
  const subtitle=error||view.kind==='error'?t('进度会保留，可以继续重试。'):ready?t('浏览网页、整理文件，现在就开始。'):view.kind==='restart'?t('重启一下，就可以开始工作。'):busy?t('你可以先去聊天，我们会继续准备。'):view.kind==='upgrade'?t('已有文件还在。更新后可以使用压缩包、表格脚本和演示文稿。'):view.kind==='stopped'&&vm.appsReady?t('你的文件和应用，都还在。'):t('浏览网页、处理文件，把想法做出来。');
  const action=t(error||view.kind==='error'?'重试':view.kind==='restart'?'重启并继续':view.kind==='stopped'?'启动电脑':view.kind==='incomplete'?'继续准备':view.kind==='upgrade'?'更新桌面':'准备桌面');
  return <div className="computer-setup" ref={content} data-state={error?'error':busy?'working':view.kind} aria-busy={busy}>
    ...
    {!ready&&!busy&&view.kind==='invite'&&<div className="setup-footnote">{t('首次约 5～15 分钟 · 需要联网')}<details><summary aria-label={t('查看准备要求')}><Icon name="info" size={13}/></summary><p>{t('初始化期间请保持应用开启。Windows 需启用硬件虚拟化和虚拟机监控程序平台。')}</p></details></div>}
    {!ready&&!busy&&view.kind==='upgrade'&&<div className="setup-footnote">{t('通常几分钟 · 需要联网')}</div>}
