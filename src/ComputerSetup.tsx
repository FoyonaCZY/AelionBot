...
function SetupProgress({vm}:{vm:VmState}){
  ...
  return <div className="setup-progress" role="progressbar" aria-label={downloading?'系统镜像下载进度':'工作电脑准备进度'} aria-valuemin={0} aria-valuemax={100} aria-valuenow={percent} aria-valuetext={percent===undefined?'正在准备':`${percent}%`}><span className={percent===undefined?'indeterminate':''} style={percent===undefined?undefined:{width:`${percent}%`}}/></div>;
}
export function ComputerStatus({vm,onOpen,minimal=false,bot}:{vm:VmState;onOpen:()=>void;minimal?:boolean;bot?:Pick<Bot,'id'|'name'|'color'|'avatarStyle'>}){
  ...
  if(minimal){
    const title=view.kind==='stopped'?'电脑已关闭':view.kind==='invite'?'准备一个工作桌面':view.kind==='download'?'正在准备系统':view.kind==='starting'?'正在启动':view.kind==='stopping'?'正在关闭':view.kind==='installing'?'正在准备应用':view.kind==='restart'?'需要重启':view.kind==='error'?'准备未完成':view.kind==='incomplete'?'继续准备桌面':'桌面已就绪';
    const label=view.working?'查看进度':view.kind==='stopped'?'启动电脑':view.kind==='invite'?'开始设置':view.kind==='error'?'重试':view.kind==='restart'?'重启电脑':'继续设置';
    return <section className="computer-rest-card" data-state={view.kind} aria-label="工作电脑状态"><div className={`computer-rest-visual ${view.working?'':'is-sleeping'}`} aria-hidden="true">
    ...
    </div><span className="computer-rest-title" role="status" aria-live="polite">{title}</span>{view.working&&<SetupProgress vm={vm}/>}<button type="button" className="computer-rest-start" onClick={onOpen} aria-label={view.working?'查看工作电脑准备进度':computerSetupActionLabel(vm)}>{!view.working&&<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" aria-hidden="true"><path d="M12 3v9m-5-7a9 9 0 1 0 10 0"/></svg>}{label}</button></section>;
  }
...
export function ComputerSetup({vm,disabled,bot,onClose,onReady,onNotify}:{vm:VmState;disabled:boolean;bot?:Pick<Bot,'id'|'name'|'color'|'avatarStyle'>;onClose:()=>void;onReady:()=>void;onNotify:(message:string)=>void}){
  ...
  const title=error?'准备暂时停住了':pending?'正在唤醒桌面':({invite:'给伙伴一个桌面',stopped:'让伙伴开始工作',download:'正在准备新桌面',starting:'桌面正在醒来',stopping:'让桌面休息一下',installing:'把工具准备好',restart:'就差最后一步',error:'准备暂时停住了',incomplete:'接着准备桌面',ready:'一切就绪'}[view.kind]);
  const subtitle=error||view.kind==='error'?'进度会保留，可以继续重试。':ready?'浏览网页、整理文件，现在就开始。':view.kind==='restart'?'重启一下，就可以开始工作。':busy?'你可以先去聊天，我们会继续准备。':view.kind==='stopped'&&vm.appsReady?'你的文件和应用，都还在。':'浏览网页、处理文件，把想法做出来。';
  const action=error||view.kind==='error'?'重试':view.kind==='restart'?'重启并继续':view.kind==='stopped'?'启动电脑':view.kind==='incomplete'?'继续准备':'准备桌面';
  return <div className="computer-setup" ref={content} data-state={error?'error':busy?'working':view.kind} aria-busy={busy}>
    ...
    {busy?<div className="setup-current" role="status"><div className="setup-current-label"><span>{pending?'正在连接':view.kind==='download'?'下载系统':view.kind==='installing'?'准备应用':view.kind==='stopping'?'保存并关闭':'连接桌面'}</span><span>{view.kind==='download'&&Number.isFinite(vm.progress)?`${Math.round(Math.min(1,Math.max(0,vm.progress!))*100)}%`:view.kind==='installing'&&Number.isFinite(vm.installation?.percent)?`${Math.floor(vm.installation!.percent!)}%`:'请稍候'}</span></div><SetupProgress vm={vm}/></div>:!ready&&firstTime&&view.kind==='invite'?<div className="setup-capabilities" aria-label="工作电脑可以使用"><span><Icon name="globe" size={15}/>浏览器</span><span><Icon name="folder" size={15}/>文件</span><span><Icon name="file" size={15}/>办公应用</span></div>:null}
    {(error||view.kind==='error')&&<details className="setup-diagnostics"><summary>查看原因</summary><p className="setup-error" role="alert">{error||vm.lastError||'初始化未完成，请重试。'}</p></details>}
    {view.working&&vm.detail&&<details className="setup-diagnostics"><summary>查看进度详情</summary><p>{vm.detail}</p></details>}
    {disabled&&!ready&&!busy&&<p className="setup-requirements">请先结束正在运行的 Bot 任务</p>}
    <div className="setup-actions">{busy?<button className="primary-button setup-background" onClick={onClose}>后台继续<Icon name="arrow" size={15}/></button>:<><button className="secondary-button" onClick={onClose}>{ready?'关闭':'稍后'}</button><button className="primary-button" disabled={!ready&&disabled} onClick={ready?onReady:()=>void run()}>{ready?'打开桌面':action}<Icon name={ready?'arrow':view.kind==='restart'?'restart':'arrow'} size={15}/></button></>}</div>
    {!ready&&firstTime&&!busy&&view.kind==='invite'&&<div className="setup-footnote">首次约 5～15 分钟 · 需要联网<details><summary aria-label="查看准备要求"><Icon name="info" size={13}/></summary><p>初始化期间请保持应用开启。Windows 需启用硬件虚拟化和虚拟机监控程序平台。</p></details></div>}
    {busy&&<p className="setup-footnote">保持 AelionBot 开启即可</p>}
  </div>;
}
