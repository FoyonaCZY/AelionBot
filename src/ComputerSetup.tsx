...
export function ComputerStatus({vm,onOpen,minimal=false,bot}:{vm:VmState;onOpen:()=>void;minimal?:boolean;bot?:Pick<Bot,'id'|'name'|'color'|'avatarStyle'>}){
  ...
  if(minimal){
    const title=t(view.kind==='stopped'?'电脑已关闭':view.kind==='invite'?'初始化工作电脑':view.kind==='download'?'正在准备系统':view.kind==='starting'?'正在启动':view.kind==='stopping'?'正在关闭':view.kind==='installing'?'正在准备应用':view.kind==='restart'?'需要重启':view.kind==='error'?'准备未完成':view.kind==='incomplete'?'继续准备桌面':view.kind==='upgrade'?'更新工作环境':'桌面已就绪');
  ...
  return <section className="computer-setup-card" aria-label={t('工作电脑状态')}>
    ...
    {view.kind==='upgrade'?<p>{t('更新将安装压缩包、表格和演示文稿工具，保留现有文件。')}</p>:view.kind==='restart'?<p>{t('应用已安装，请重启工作电脑继续。')}</p>:view.working&&vm.detail?<p>{vm.detail}</p>:null}
...
export function ComputerSetup({vm,disabled,bot,onClose,onReady,onNotify}:{vm:VmState;disabled:boolean;bot?:Pick<Bot,'id'|'name'|'color'|'avatarStyle'>;onClose:()=>void;onReady:()=>void;onNotify:(message:string)=>void}){
  ...
  const title=t(error?'工作电脑准备未完成':pending?'正在启动工作电脑':view.title);
  const subtitle=view.kind==='upgrade'?t('更新将安装压缩包、表格和演示文稿工具，保留现有文件。'):view.kind==='restart'?t('应用已安装，请重启工作电脑继续。'):undefined;
  ...
  return <div className="computer-setup" ref={content} data-state={error?'error':busy?'working':view.kind} aria-busy={busy}>
    ...
    <div className="setup-intro"><h3 aria-live="polite">{title}</h3>{subtitle&&<p>{subtitle}</p>}</div>
