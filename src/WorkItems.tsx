...
function WorkCard({item,bots}:{item:WorkItem;bots:Bot[]}){
  ...
  return <section className={`work-card companion-surface work-${item.status} ${open?'is-open':''}`} data-work-id={item.id} aria-label={t('{kind}：{objective}',{kind:t(item.kind==='plan'?'计划':'目标'),objective:item.objective})}>
    <div className="work-card-heading"><button type="button" className="work-card-toggle" aria-expanded={open} aria-controls={open?bodyId:undefined} aria-label={t('{action}{kind}：{objective}',{action:t(open?'收起':'展开'),kind:t(item.kind==='plan'?'计划':'目标'),objective:item.objective})} onClick={()=>setOpen(!open)}>
      <CompanionBadge kind={item.kind} activity={activity}/><span className="work-heading-copy"><span className="work-heading-meta"><span className="work-kind">{t(item.kind==='plan'?'计划':'目标')}</span><span className="work-status companion-status">{t(labels[item.status])}</span></span>{!open&&<span className="work-card-title" title={item.objective}>{(steps.find(step=>step.status==='working')?.title)||item.objective}</span>}</span>
      {steps.length>0&&<span className="work-progress-count" aria-label={t('已处理 {done} 步，共 {total} 步',{done,total:steps.length})}><span aria-hidden="true">{done} / {steps.length}</span></span>}<span className="work-card-chevron"><Icon name={open?'down':'arrow'} size={13}/></span>
    </button>{item.scope.kind==='group'&&bot&&<span className="work-owner" title={bot.name}><Avatar bot={bot} size={22}/></span>}</div>
    {open&&<div className="work-card-body" id={bodyId}><p className="work-objective">{item.objective}</p>{steps.length>0&&<ol className="work-steps">{steps.map((step,index)=><li key={step.id} data-status={step.status}><span className="work-step-mark" role="img" aria-label={step.status==='done'?t('已完成'):step.status==='skipped'?t('已跳过'):step.status==='working'?t('进行中'):t('第 {index} 步',{index:index+1})}>{step.status==='done'?<Icon name="check" size={12}/>:step.status==='skipped'?'−':step.status==='working'?<span className="work-step-orbit"/>:index+1}</span><div><strong>{step.title}</strong>{step.acceptance&&<p>{step.acceptance}</p>}{step.note&&<p className="work-step-note">{step.note}</p>}</div></li>)}</ol>}{item.summary&&<p className="work-summary">{item.summary}</p>}{item.workspaceDir&&<div className="work-directory" title={item.workspaceDir}><Icon name="folder" size={12}/><span>{item.workspaceDir}</span></div>}</div>}
    {item.reason&&!ended&&<p className="work-reason">{item.reason}</p>}
    {!ended&&<div className="work-card-actions"><button type="button" className="companion-button companion-quiet" disabled={pending} onClick={()=>void act('cancel')}>{t('取消')}</button>{active?<button type="button" className="companion-button" disabled={pending||['paused','blocked'].includes(item.status)} onClick={()=>void act('pause')}><Icon name="pause" size={13}/>{item.status==='paused'?t('正在暂停…'):t('暂停')}</button>:<button type="button" className="work-start companion-button companion-primary" disabled={pending} onClick={()=>void act('start')}>{pending?t('正在处理…'):item.kind==='plan'&&!item.approvedAt?(steps.length?t('开始执行'):t('继续规划')):t('继续执行')}</button>}</div>}
