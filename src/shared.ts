...
export interface ProviderModel {id:string;contextTokens?:number;reasoningEffort?:string;thinkingBudget?:number;supportsImages?:boolean;imageOutput?:boolean;imageAspect?:import('./image-types').ImageAspect;imageQuality?:import('./image-types').ImageQuality;}
...
export interface Artifact { id: string; botId: string; runId: string; path: string; name: string; size: number; modifiedAt: string; kind?: 'file' | 'web'; url?: string; urlLocation?: 'host' | 'vm'; }
...
export interface Snapshot {designer?:import('./designer-types').DesignerSnapshot;previewRequests?:import("./agent-preview").AgentPreviewRequest[];previewHistory?:import("./agent-preview").PreviewHistoryEntry[];appearance?:import("./appearance").AppearanceSettings;userProfile?:import("./user-profile").UserProfile;hostPermissionModes?:Record<string,HostPermissionMode>;platform?:string; workItems?:WorkItem[];conversationWorkspaces?:Record<string,string>; runtime?:RuntimeSettings;modelUsage?:UsageRecord[];updates?:UpdateState; scheduledTasks?:ScheduledTask[]; bots: Bot[]; messages: ChatMessage[]; runs: RunRecord[]; model: ModelConfig; providers?:ModelProvider[];defaultModel?:ModelSelection;approvalModel?:ModelSelection;botModels?:Record<string,ModelConfig>; streamingReplies?:StreamingReply[]; vm: VmState; skills: Skill[]; artifacts: Artifact[]; computer: ComputerState; dataDir: string; integrations?:IntegrationsView; interactions?:InteractionRequest[]; cognition?:CognitionView; peers?:PeerView;groups?:GroupsView;layaFeature?:import('./laya-types').LayaFeatureState; greetingBotIds?:string[]; commandPermissions?:CommandPermissionRule[]; hostWorkspace?:HostWorkspaceSettings; liveWork?:import('./live-work').LiveWorkItem[]; }
...
export interface AelionAPI {
  ...
  testImageModel(input:{selection:ModelSelection}):Promise<{protocol:import('./image-types').ImageProtocol;mediaType:string;bytes:number}>;
  ...
  searchMentionFiles(input:{designSessionId?:string;scope:import("./attachment-types").AttachmentScope;query:string}):Promise<import("./file-mentions").MentionFileSearch>;
