...
export function FilePreviewProvider({children}:{children:ReactNode}){
 const [session,setSession]=useState<Session>(),[info,setInfo]=useState<PreviewWorkbenchInfo>();
 ...
 const show=(next?:Session)=>{setInfo(undefined);setSession(next?{...next,generation:++generation.current}:undefined);};
 ...
 const send=useCallback(async(scope:AttachmentScope,input:PreviewChatInput)=>{const view=infoRef.current;if(!input.text.trim()||!view?.docked||scopeKey(view.scope)!==scopeKey(scope)||!sender.current)return false;await sender.current(input);return true;},[]);
