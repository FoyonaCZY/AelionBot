...
export function validateAnnotations(value:unknown):PreviewAnnotation[]{
 ...
 if(raw.text!==undefined&&(typeof raw.text!=='string'||raw.text.length>1000)||raw.selector!==undefined&&(typeof raw.selector!=='string'||raw.selector.length>3000)||raw.designId!==undefined&&(typeof raw.designId!=='string'||raw.designId.length>120))throw Error('标注文字过长');
 return {id:raw.id,type:raw.type,...p,color:raw.color,...(raw.w!==undefined?{w:raw.w}:{}),...(raw.h!==undefined?{h:raw.h}:{}),...(raw.end?{end:point(raw.end)}:{}),...(raw.points?{points:raw.points.map(point)}:{}),...(raw.text?{text:raw.text}:{}),...(raw.selector?{selector:raw.selector}:{}),...(raw.designId?{designId:raw.designId}:{}),...(Number.isInteger(raw.page)&&raw.page>0?{page:raw.page}:{})};});
}
export function annotationContext(annotations:PreviewAnnotation[]){return validateAnnotations(annotations).map((a,i)=>`#${i+1} ${a.type}${a.page?' · page '+a.page:''}${a.designId?' · data-design-id='+a.designId:''}${a.selector?' · '+a.selector:''}${a.text?' · '+a.text:''} [${[a.x,a.y,a.w??0,a.h??0].map(n=>n.toFixed(4)).join(', ')}]`).join('\n');}
