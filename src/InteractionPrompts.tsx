...
function PermissionCard({request,count}:{request:Permission;count:number}){
  ...
  return <section id={`interaction-${request.id}`} className={`conversation-request permission-card companion-surface ${reviewing?'approval-reviewing':''}`} tabIndex={-1} aria-label="本机操作权限">
    ...
    <div className="permission-scope">
      ...
      {details.server&&<div><span>服务</span><code>{details.server}</code></div>}
      {details.tool&&<div><span>操作</span><code>{details.tool}</code></div>}
    </div>
    {details.command!==undefined&&<div className="permission-content"><span>命令</span><pre tabIndex={0}>{details.command}</pre></div>}
    {details.content!==undefined&&<div className="permission-content"><span>写入内容</span><pre tabIndex={0}>{details.content||'（空文件）'}</pre></div>}
...
function QuestionCard({request}:{request:Questions}){
  ...
  return <form className="conversation-request question-card companion-surface" id={`interaction-${request.id}`} aria-label="回答 Bot 的问题" onSubmit={event=>{event.preventDefault();void send();}}>
    <header><CompanionBadge kind="permission" activity="waiting"/><div className="request-heading-copy"><h2>需要你的回答</h2></div></header>
    <div className="permission-body">{request.questions.map(question=><fieldset key={question.id} disabled={sending}><legend>{question.title}</legend>{question.options?.map(option=><label className="question-option" key={option}><input type="radio" name={`${request.id}-${question.id}`} checked={answers[question.id]===option} onChange={()=>setAnswers({...answers,[question.id]:option})}/><span>{option}</span></label>)}<textarea aria-label={`${question.title}：填写回答`} placeholder={question.options?'也可以填写自己的答案':'填写回答'} value={question.options?.includes(answers[question.id])?'':answers[question.id]||''} maxLength={4000} rows={2} onChange={event=>setAnswers({...answers,[question.id]:event.target.value})}/></fieldset>)}{error&&<p className="permission-error" role="alert">{error}</p>}</div>
    <footer><button className="companion-button companion-primary" disabled={sending||request.questions.some(question=>!answers[question.id]?.trim())}>{sending?'正在发送…':'发送回答'}</button></footer>
  </form>;
}
...
export function InteractionNotifications({requests,bots,onView}:{requests:InteractionRequest[];bots:Bot[];onView:(request:InteractionRequest)=>void}){
  ...
  return <aside className="interaction-notification companion-surface" role="status" aria-live="polite" onMouseEnter={()=>setPaused(true)} onMouseLeave={()=>setPaused(false)} onFocus={()=>setPaused(true)} onBlur={event=>{if(!event.currentTarget.contains(event.relatedTarget as Node|null))setPaused(false);}}>
    <button className="interaction-notification-open" onClick={()=>{setVisible(undefined);onView(request);}} aria-label={`查看 ${bot?.name||'Bot'} 的${request.kind==='host_permission'?'权限请求':request.kind==='user_input'?'问题':'接管请求'}`}>
      {bot&&<Avatar bot={bot} size={34} activity="waiting"/>}<span><strong>{bot?.name||'Bot'}</strong><small>{request.kind==='host_permission'?'等待你的本机操作许可':request.kind==='user_input'?'有问题需要你回答':'需要你接管工作电脑'}</small></span><span className="notification-view">查看<Icon name="arrow" size={11}/></span>
