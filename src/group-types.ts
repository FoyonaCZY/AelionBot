...
export interface GroupMessage {designSessionId?:string;previewPrompt?:string;reply?:MessageReply;workItemId?:string;workspaceDir?:string|null;scheduled?:ScheduledTrigger;attachments?:Attachment[];pins?:MessagePin[];reaction?:PinEvent;event?:GroupLifecycleEvent;id:string;seq:number;groupId:string;sender:GroupSender;kind:'message'|'system'|'continue'|'reaction'|'progress';content:string;time:string;rootId?:string;replyTo?:string;runIds?:string[];mentions?:BotMention[];}
export interface GroupTask {id:string;key:string;title:string;ownerId?:string;status:'open'|'working'|'blocked'|'paused'|'completed';summary:string;reason?:string;sourceMessageId:string;revision:number;createdAt:string;updatedAt:string;runIds:string[];}
export interface GroupOutbox {id:string;botId:string;groupId:string;rootId:string;runId:string;key:string;content:string;mentions:BotMention[];attachments?:Attachment[];kind:'message'|'progress';replyTo?:string;status:'pending'|'sent';createdAt:string;messageId?:string;designSessionId?:string;}
export interface GroupRoom {id:string;name:string;members:GroupMember[];createdBy:GroupSender;createdAt:string;updatedAt:string;messages:GroupMessage[];lastReadSeq:number;activeRootId?:string;tasks?:GroupTask[];}
...
export interface GroupDelivery {retryRunId?:string;layaMode?:'v2';layaDecisionId?:string;id:string;groupId:string;messageId:string;recipientId:string;rootId:string;status:GroupDeliveryStatus;createdAt:string;reason?:string;runId?:string;replyMessageId?:string;}
export interface GroupLayaDecision {sourceId:string;messageId:string;actorId:string;choice:string;appliedChoice?:'observe'|'participate'|'reply'|'act';adjustment?:string;probabilities?:Record<string,number>;confidence?:number;criteria?:Record<string,string>;features?:{events:number;recent:number;mentioned:boolean;ownTask:boolean};input?:unknown;runtime:string;model:string;elapsedMs:number;time?:string;deliveryStatus:GroupDeliveryStatus;replyMessageId?:string;runId?:string;reason?:string;}
...
export interface GroupPage {tasks?:GroupTask[];pins?:Record<string,MessagePin[]>;group:GroupSummary;messages:GroupMessage[];deliveries:GroupDelivery[];laya?:{runtime?:string;enabled:boolean;ready?:boolean;decisions:GroupLayaDecision[]};before?:string;}
