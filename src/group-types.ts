...
export interface GroupMessage {previewPrompt?:string;reply?:MessageReply;workItemId?:string;workspaceDir?:string|null;scheduled?:ScheduledTrigger;attachments?:Attachment[];pins?:MessagePin[];reaction?:PinEvent;event?:GroupLifecycleEvent;delegation?:{fromBotId:string;mode:'assist'|'handoff';task:string};id:string;seq:number;groupId:string;sender:GroupSender;kind:'message'|'system'|'continue'|'reaction'|'progress';content:string;time:string;rootId?:string;replyTo?:string;runIds?:string[];mentions?:BotMention[];}
export interface GroupRoom {id:string;name:string;members:GroupMember[];createdBy:GroupSender;createdAt:string;updatedAt:string;messages:GroupMessage[];lastReadSeq:number;coordinatorBotId?:string;backupCoordinatorBotIds?:string[];activeRootId?:string;}
export interface GroupRound {id:string;originKey?:string;groupId:string;request:string;ownerBotId?:string;status:'active'|'limited'|'stopped';createdAt:string;botMessages:number;botCounts:Record<string,number>;decisions:number;createdGroups:number;delegations?:Record<string,string>;reason?:string;repetitions?:number;}
...
export interface GroupDelivery {retryRunId?:string;delegatedByBotId?:string;id:string;groupId:string;messageId:string;recipientId:string;rootId:string;status:GroupDeliveryStatus;createdAt:string;reason?:string;runId?:string;replyMessageId?:string;}
export interface GroupRunOrigin {groupId:string;rootId:string;deliveryId:string;}
export interface GroupSummary {id:string;name:string;members:GroupMember[];createdBy:GroupSender;updatedAt:string;preview:string;unread:number;lastSeq:number;pending:number;coordinatorBotId?:string;round?:{id:string;status:GroupRound['status'];ownerBotId?:string;botMessages:number;reason?:string};activities?:Array<{botId:string;phase:'deciding'|'running'|'updating'}>;activity?:{botId:string;phase:'deciding'|'running'};}
