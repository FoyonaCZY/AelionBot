...
export function GroupTaskMessage({message,view,onOpen}:{message:ChatMessage;view?:GroupsView;onOpen:(id:string)=>void}){
  ...
  return <><MessageTime id={message.id} time={message.time}/><div className="peer-task-message group-task-message"><button className="peer-task-source" disabled={!room} onClick={()=>onOpen(source.groupId)}><Icon name="message" size={17}/><span>{source.continuation?'继续来自':'来自'} <strong>{room?.name||source.name}</strong> 的群任务</span><Icon name="arrow" size={12}/></button><p><MentionContent content={message.content} mentions={message.mentions}/></p></div></>;
}
...
export function GroupConversation({group,state,avatarActivities,draft,onDraft,onManage,onTakeover,onOpenFile,onSaveFile,visible}:{group:GroupSummary;state:Snapshot;avatarActivities?:BotActivities;draft:ComposerDraft;onDraft:(draft:ComposerDraft)=>void;onManage:()=>void;onTakeover:(request:Extract<InteractionRequest,{kind:'vm_takeover'}>)=>Promise<void>;onOpenFile:(file:FileItem&{botId:string})=>void;onSaveFile:(file:FileItem&{botId:string})=>void;visible:boolean}){
  ...
  return <><header className="chat-header drag"><button className="bot-heading no-drag" onClick={onManage}><GroupAvatar group={group} activities={avatarActivities}/><strong>{group.name}</strong><span className="group-member-count">{members.length+1} 人</span></button><div className="header-actions no-drag"><button className="icon-button" aria-label="群聊设置" onClick={onManage}><Icon name="settings"/></button></div></header>
    <ConversationTimeProvider messages={page?.messages||[]}><section ref={body} className="messages group-messages" onScroll={event=>{const element=event.currentTarget;follow.current=element.scrollHeight-element.scrollTop-element.clientHeight<90;}}>{page?.before&&<button className="peer-chat-load" disabled={loading} onClick={()=>void older()}>加载更早消息</button>}{page?.messages.map(message=>{
      if(message.scheduled)return <Fragment key={message.id}><MessageTime id={message.id} time={message.time}/><div className="scheduled-trigger" data-group-message-id={message.id}><div><Icon name="clock" size={15}/><span>定时任务 · {message.scheduled.title}</span></div><p>{message.content}</p></div></Fragment>;
      ...
      return <Fragment key={message.id}><MessageTime id={message.id} time={message.time}/><article className={`group-message ${message.sender.kind==='user'?'from-user':''}`} data-group-message-id={message.id}>{bot&&<Avatar bot={bot} size={31}/>}<div className="group-message-copy"><div className="group-message-author">{(message.sender.kind==='user'?userDisplayName(state.userProfile):bot?.name||message.sender.name)}</div><MessageActions messageId={message.id} content={formatted.content||attachmentSummary(message.attachments)} pins={message.pins} bubbleClassName="group-message-bubble markdown" onReply={()=>onDraft({...draftRef.current,reply:messageReply({...message,content:formatted.content},(message.sender.kind==='user'?userDisplayName(state.userProfile):bot?.name||message.sender.name),message.sender.id)})} onPin={input=>window.aelion.pinGroup({...input,groupId:group.id})}>{message.reply&&<MessageQuote reply={message.reply}/>}<MentionContent content={formatted.content} mentions={formatted.mentions} markdown/><AttachmentList files={message.attachments}/></MessageActions>{artifacts.map(file=><FileCard key={file.id} file={file} onOpen={()=>onOpenFile(file)} onSave={()=>onSaveFile(file)} disabled={state.vm.status!=='ready'}/>)}</div></article></Fragment>;
    })}{members.filter(bot=>group.activities?.some(activity=>activity.botId===bot.id)||state.runs.some(run=>run.botId===bot.id&&run.groupOrigin?.groupId===group.id&&run.status==='running')).map(bot=>{const run=[...state.runs].reverse().find(run=>run.botId===bot.id&&run.groupOrigin?.groupId===group.id&&run.status==='running'),request=requests.find(request=>request.botId===bot.id);return <BotWorkingStatus key={bot.id} bot={bot} showName step={liveBotStep(state.messages,run,request?.kind,request?.kind==='host_permission'&&request.approval?.phase==='reviewing')||{phase:'thinking',label:group.activities?.find(activity=>activity.botId===bot.id)?.phase==='updating'?'正在整理回复':'正在思考'}}/>;})}{page&&!page.messages.some(message=>message.kind==='message'||message.kind==='progress')&&!group.activities?.length&&<div className="group-empty">发条消息，开始一起协作</div>}</section></ConversationTimeProvider>
