...
export function GroupConversation({group,state,avatarActivities,draft,onDraft,onManage,onTakeover,onOpenFile,onSaveFile,onOpenPreviewEntry,visible}:{group:GroupSummary;state:Snapshot;avatarActivities?:BotActivities;draft:ComposerDraft;onDraft:(draft:ComposerDraft)=>void;onManage:()=>void;onTakeover:(request:Extract<InteractionRequest,{kind:'vm_takeover'}>)=>Promise<void>;onOpenFile:(file:FileItem&{botId:string})=>void;onSaveFile:(file:FileItem&{botId:string})=>void;onOpenPreviewEntry:(entry:PreviewHistoryEntry)=>void;visible:boolean}){
  ...
  const continueTask=async(task:GroupTask)=>{if(sendLock.current)return;sendLock.current=true;setSending(true);try{await window.aelion.sendGroup({id:group.id,message:`请继续群任务：${task.title}（taskId: ${task.id}）。先核对自己的执行检查点和已有结果，只完成剩余部分；完成后写明验收依据，无法完成则记录具体阻碍。`});if(active.current)setError('');}catch(error){if(active.current)setError(errorText(error));}finally{sendLock.current=false;if(active.current)setSending(false);}};
  ...
  return <><header className="chat-header drag"><button className="bot-heading no-drag" onClick={onManage}><GroupAvatar group={group} activities={avatarActivities}/><strong>{group.name}</strong><span className="group-member-count">{t('{count} 人',{count:members.length+1})}</span></button><div className="header-actions no-drag"><button className="icon-button" aria-label={t('群聊设置')} onClick={onManage}><Icon name="settings"/></button></div></header>
    {!!page?.tasks?.length&&<details className="group-commitments"><summary>{t('群任务')} · {page.tasks.filter(task=>task.status!=='completed').length} {t('未完成')}</summary><ul>{page.tasks.map(task=><li key={task.id}><div><strong>{task.title}</strong><span>{state.bots.find(bot=>bot.id===task.ownerId)?.name||t('待认领')} · {t(({open:'待认领',working:'进行中',blocked:'有阻碍',paused:'已暂停',completed:'已完成'} as const)[task.status])}</span></div>{task.summary&&<p>{task.summary}</p>}{task.reason&&<p>{task.reason}</p>}{['open','blocked','paused'].includes(task.status)&&<button type="button" className="group-task-continue" disabled={sending} onClick={()=>void continueTask(task)}>{sending?t('正在处理…'):t('继续任务')}</button>}</li>)}</ul></details>}
