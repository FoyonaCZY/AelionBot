...
const compact=(value:number)=>new Intl.NumberFormat(currentLanguage(),{notation:'compact',maximumFractionDigits:1}).format(value);
...
function UsageTimeline({buckets,metric}:{buckets:UsageBucket[];metric:Metric}){
  ...
  return <div className="usage-timeline">
    <div className="usage-chart-detail" aria-live="polite">{selectedRow?<><span>{selectedRow.key}</span><strong>{selectedValue===undefined?t('未返回用量'):t('{value} tokens',{value:number(selectedValue)})}</strong><span>{t('{count} 次请求',{count:selectedRow.requests})}</span></>:<span>{t('悬停或使用键盘查看各时段')}</span>}</div>
    <svg viewBox={`0 0 ${width} ${height}`} role="group" aria-label={t('{metric}时间趋势',{metric:t(metricNames[metric])})} onMouseLeave={()=>setSelected(undefined)}>
      ...
      {buckets.map((row,index)=><rect key={row.key} x={left+index*plotWidth/buckets.length} y={top} width={plotWidth/buckets.length} height={plotHeight} fill="transparent" tabIndex={index===(selected??0)?0:-1} role="button" aria-label={t('{key}，{metric} {value}，{count} 次请求',{key:row.key,metric:t(metricNames[metric]),value:metricValue(row,metric)===undefined?t('未返回'):number(row[metric]),count:row.requests})} onMouseEnter={()=>setSelected(index)} onFocus={()=>setSelected(index)} onBlur={()=>setSelected(undefined)} onKeyDown={event=>{if(!['ArrowLeft','ArrowRight','Home','End'].includes(event.key))return;event.preventDefault();const next=event.key==='Home'?0:event.key==='End'?buckets.length-1:Math.max(0,Math.min(buckets.length-1,index+(event.key==='ArrowRight'?1:-1)));(event.currentTarget.parentElement?.querySelectorAll('rect')[next] as SVGElement)?.focus();}}/>)}
    </svg>
  </div>;
}
...
function Breakdown({title,groups,onChoose,model=false}:{title:string;groups:UsageGroup[];onChoose:(row:UsageGroup)=>void;model?:boolean}){
  ...
  return <section className="usage-breakdown"><h3>{title}</h3>{groups.length?((expanded?groups:groups.slice(0,6)).map(row=><button type="button" className="usage-bar-row" key={row.key} onClick={()=>onChoose(row)} aria-label={t('筛选{kind} {name}',{kind:model?t('模型'):'Provider',name:model?row.model+' · '+row.name:row.name})}><span className="usage-bar-label"><span title={model?row.model:row.name}>{model?row.model:row.name}</span>{model&&<small title={row.name}>{row.name}</small>}</span><span className="usage-bar-track"><i style={{width:`${row.totalTokens/maximum*100}%`}}/></span><span className="usage-bar-value">{row.reportedRequests?compact(row.totalTokens):'—'}</span></button>)):<p className="usage-empty-small">{t('暂无记录')}</p>}{groups.length>6&&<button type="button" className="text-button" onClick={()=>setExpanded(value=>!value)}>{expanded?t('收起'):t('展开全部 {count} 项',{count:groups.length})}</button>}</section>;
}
...
export function UsageSettings({state}:{state:Snapshot}){
  ...
  return <div className="usage-settings">
    <div className="usage-filters">
      <label><span>{t('时间')}</span><Select aria-label={t('用量时间范围')} value={preset} onChange={event=>{const value=event.target.value;setPreset(value);if(value!=='custom')update({...range(Number(value)),granularity:value==='1'?'hour':'day'});}}><option value="1">{t('今天')}</option><option value="7">{t('最近 7 天')}</option><option value="30">{t('最近 30 天')}</option><option value="90">{t('最近 90 天')}</option><option value="custom">{t('自定义')}</option></Select></label>
      <label><span>Provider</span><Select aria-label={t('用量 Provider')} value={query.providerId||''} onChange={event=>update({providerId:event.target.value||undefined,model:undefined})}><option value="">{t('全部 Provider')}</option>{(report?.providers||state.providers||[]).map(provider=><option key={provider.id} value={provider.id}>{provider.name}</option>)}</Select></label>
      <label><span>{t('模型')}</span><Select aria-label={t('用量模型')} value={query.model||''} onChange={event=>update({model:event.target.value||undefined})}><option value="">{t('全部模型')}</option>{[...new Set([...(report?.models||[]),...(query.model?[query.model]:[])])].map(model=><option key={model}>{model}</option>)}</Select></label>
      <label><span>{t('用途')}</span><Select aria-label={t('用量用途')} value={query.purpose||''} onChange={event=>update({purpose:event.target.value||undefined})}><option value="">{t('全部用途')}</option>{[...new Set([...(report?.purposes||[]),...(query.purpose?[query.purpose]:[])])].map(purpose=><option key={purpose} value={purpose}>{t(purposes[purpose]||purpose)}</option>)}</Select></label>
    </div>
    ...
    {error?<div className="usage-query-error" role="alert">{error}</div>:!totals?<div className="usage-loading" role="status">{t('正在统计用量…')}</div>:<>
      <div className="usage-metrics">
        ...
        <div><span>{t('输入')}</span><strong title={number(totals.inputTokens)}>{totals.inputReports||!totals.requests?compact(totals.inputTokens):'—'}</strong></div>
        <div><span>{t('输出')}</span><strong title={number(totals.outputTokens)}>{totals.outputReports||!totals.requests?compact(totals.outputTokens):'—'}</strong></div>
        <div><span>{t('缓存命中率')}</span><strong>{percent(totals)}</strong></div>
      </div>
      {!totals.requests?<div className="usage-empty"><svg width="42" height="42" viewBox="0 0 40 40" fill="none" aria-hidden="true"><path d="M7 31h26M11 26V17m9 9V8m9 18V12" stroke="#aaa3bc" strokeWidth="2.5" strokeLinecap="round"/></svg><strong>{t('这段时间还没有用量记录')}</strong><span>{t('调整筛选，或与 Bot 聊天后再来查看。')}</span></div>:<>
        <section className="usage-chart-card"><div className="usage-chart-heading"><h3>{t('用量趋势')}</h3><div><Select aria-label={t('用量图表指标')} value={metric} onChange={event=>setMetric(event.target.value as Metric)}>{Object.entries(metricNames).map(([value,label])=><option key={value} value={value}>{t(label)}</option>)}</Select><Select aria-label={t('用量时间粒度')} value={query.granularity} onChange={event=>update({granularity:event.target.value as UsageQuery['granularity']})}><option value="hour" disabled={(new Date(query.to).getTime()-new Date(query.from).getTime())>30*86400000}>{t('按小时')}</option><option value="day">{t('按天')}</option><option value="month">{t('按月')}</option></Select></div></div><UsageTimeline buckets={report!.buckets} metric={metric}/></section>
        <div className="usage-breakdowns"><Breakdown title={t('按 Provider')} groups={report!.byProvider} onChoose={row=>update({providerId:row.providerId,model:undefined})}/><Breakdown title={t('按模型')} model groups={report!.byModel} onChoose={row=>update({providerId:row.providerId,model:row.model})}/></div>
        <div className="usage-table-wrap"><table className="usage-table"><caption>{t('模型明细')}</caption><thead><tr><th>{t('模型 / Provider')}</th><th>{t('请求')}</th><th>{t('输入')}</th><th>{t('输出')}</th><th>{t('缓存读取')}</th><th>{t('命中率')}</th></tr></thead><tbody>{report!.byModel.map(row=><tr key={row.key}><td><strong>{row.model}</strong><small>{row.name}</small></td><td>{number(row.requests)}</td><td>{row.inputReports?number(row.inputTokens):'—'}</td><td>{row.outputReports?number(row.outputTokens):'—'}</td><td>{number(row.cachedTokens)}</td><td>{percent(row)}</td></tr>)}</tbody></table></div>
      </>}
    </>}
  </div>;
}
