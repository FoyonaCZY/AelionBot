...
export function UsageSettings({state}:{state:Snapshot}){
  ...
  return <div className="usage-settings">
    ...
    {error?<div className="usage-query-error" role="alert">{error}</div>:!totals?<div className="usage-loading" role="status">{t('正在统计用量…')}</div>:<>
      ...
      {!totals.requests?<div className="usage-empty"><svg width="42" height="42" viewBox="0 0 40 40" fill="none" aria-hidden="true"><path d="M7 31h26M11 26V17m9 9V8m9 18V12" stroke="#aaa3bc" strokeWidth="2.5" strokeLinecap="round"/></svg><strong>{t('这段时间还没有用量记录')}</strong></div>:<>
