...
export function MessageQuote({reply,onCancel}:{reply:MessageReply;onCancel?:()=>void}){
  ...
  return <div className={`message-quote ${onCancel?'is-draft':''}`}><button type="button" className="message-quote-content" onClick={jump} title={t('查看原消息')} aria-label={t('引用 {author} 的消息：{excerpt}',{author:reply.author,excerpt:reply.excerpt})}><span className="message-quote-author">{reply.author}</span><span className="message-quote-excerpt">{missing?t('原消息不在当前已加载的记录中'):reply.excerpt}</span></button>{onCancel&&<button type="button" className="message-quote-cancel" onClick={onCancel} aria-label={t('取消回复')} title={t('取消回复')}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" aria-hidden="true"><path d="m6 6 12 12M18 6 6 18"/></svg></button>}</div>;
}
