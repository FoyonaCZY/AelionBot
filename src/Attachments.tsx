...
const size=(value:number)=>value<1024?`${value} B`:value<1024*1024?`${Math.ceil(value/1024)} KB`:`${(value/1024/1024).toFixed(1)} MB`;
...
function AttachmentCard({file,compact,onRemove,onOpen}:{file:Attachment;compact:boolean;onRemove?:()=>void;onOpen:()=>void}){
  ...
  return <article className={`attachment-card ${compact?'attachment-tag':''} ${image?'has-image':''}`} data-attachment-id={file.id}>
    <button type="button" className="attachment-open" onClick={onOpen} title={file.name} aria-label={`预览附件 ${file.name}`}>
      {image?<img src={image} alt=""/>:<span className="attachment-file-icon"><AttachmentIcon/></span>}<span className="attachment-name"><strong>{file.name}</strong>{!compact&&<small>{size(file.size)}</small>}</span>
    </button>
    {onRemove&&<button type="button" className="attachment-remove" aria-label={`移除附件 ${file.name}`} onClick={onRemove}>×</button>}
  </article>;
}
...
function AttachmentPreview({file,onClose}:{file:Attachment;onClose:()=>void}){
  ...
  useEffect(()=>{const key=(event:KeyboardEvent)=>{if(event.key==='Escape'){event.preventDefault();event.stopImmediatePropagation();onClose();}};window.addEventListener('keydown',key,true);void window.aelion.setWindowDimmed?.(true).catch(()=>{});return()=>{window.removeEventListener('keydown',key,true);void window.aelion.setWindowDimmed?.(Boolean(document.querySelector('[aria-modal="true"]'))).catch(()=>{});};},[onClose]);
