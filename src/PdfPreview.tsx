...
function PdfPage({pdf,page,width,aspectRatio,thumbnail=false}:{pdf:PDFDocumentProxy;page:number;width:number;aspectRatio?:number;thumbnail?:boolean}){
  ...
  return <div className="fp-pdf-page" style={{width,aspectRatio:aspectRatio??ratio}}>{error?<p role="alert">{t('此页显示失败：')}{error}</p>:<><canvas ref={canvas} aria-label={t('第 {page} 页',{page})}/>{!thumbnail&&<div ref={text} className="textLayer"/>}</>}</div>;
}
function Thumbnail({pdf,page,current,onSelect}:{pdf:PDFDocumentProxy;page:number;current:boolean;onSelect:()=>void}){
  ...
  return <button ref={ref} className="fp-thumbnail" aria-label={t('跳转到第 {page} 页',{page})} aria-current={current?'page':undefined} onClick={onSelect}>{visible?<PdfPage pdf={pdf} page={page} width={112} thumbnail/>:<span className="fp-thumb-placeholder"/>}<span>{page}</span></button>;
}
export default function PdfPreview({src,name,slides}:{src:string;name:string;slides:boolean}){
  const {t}=useI18n();
  const immersive=useImmersivePreview(),[sizing,setSizing]=useState<'width'|'fit'>(slides?'fit':'width');
  ...
  const stage=useRef<HTMLDivElement>(null),room=usePreviewViewport(stage,Boolean(pdf),padding);
  const drag=useRef<{x:number;y:number;left:number;top:number}|undefined>(undefined);
  useEffect(()=>{
    let active=true;setError('');setPdf(undefined);setPage(1);setInput('1');setZoom(null);setSizing(slides?'fit':'width');
    ...
    task.onPassword=()=>{if(active)setError(t('此文档需要密码，请保存原文件后打开。'));};
  ...
  const ratioX=room.width/pageSize.width,ratioY=room.height/pageSize.height;
  const fit=sizing==='width'?ratioX:Math.min(ratioX,ratioY),scale=zoom??fit,width=Math.max(1,pageSize.width*scale),canPan=slides&&(width>room.width+1||pageSize.height*scale>room.height+1);
  const presets=[.25,.5,.75,1,1.25,1.5,2,3],zoomValue=zoom===null?sizing:String(zoom);
  const chooseZoom=(value:string)=>{if(value==='width'||value==='fit'){setSizing(value);setZoom(null);}else setZoom(Number(value));};
  useLayoutEffect(()=>{const el=stage.current;if(el&&zoom===null){el.scrollLeft=(el.scrollWidth-el.clientWidth)/2;el.scrollTop=0;}},[sizing,zoom,width,page,room.width,room.height]);
  if(error)return <div className="fp-state" role="alert"><strong>{t('文档暂时无法显示')}</strong><p>{error}</p></div>;
  if(!pdf)return <div className="fp-state" role="status"><span className="fp-loading"/><p>{t('正在读取页面…')}</p></div>;
  return <><PreviewToolbar><div className="fp-tools"><button aria-label={t('页面缩略图')} title={t('页面缩略图')} aria-pressed={outline} onClick={()=>setOutline(!outline)}><PreviewIcon name="pages"/></button><button aria-label={t('上一页')} title={t('上一页')} disabled={page===1} onClick={()=>go(page-1)}><PreviewIcon name="left"/></button><input className="fp-page-input" aria-label={t('页码')} inputMode="numeric" value={input} onChange={event=>setInput(event.target.value)} onBlur={()=>go(Number(input))} onKeyDown={event=>{if(event.key==='Enter')go(Number(input));}}/><span className="fp-page-total">/ {pdf.numPages}</span><button aria-label={t('下一页')} title={t('下一页')} disabled={page===pdf.numPages} onClick={()=>go(page+1)}><PreviewIcon name="right"/></button></div><div className="fp-tools"><button aria-label={t('缩小页面')} title={t('缩小')} disabled={scale<=.25} onClick={()=>setZoom(Math.max(.25,scale-.25))}><PreviewIcon name="minus"/></button><Select className="fp-zoom-picker" aria-label={t('显示比例')} value={zoomValue} onChange={event=>chooseZoom(event.target.value)}><option value="width">{t('适应宽度')}</option><option value="fit">{t('整页')}</option>{zoom!==null&&!presets.includes(zoom)&&<option value={zoom}>{Math.round(zoom*100)}%</option>}{presets.map(value=><option key={value} value={value}>{Math.round(value*100)}%</option>)}</Select><button aria-label={t('放大页面')} title={t('放大')} disabled={scale>=3} onClick={()=>setZoom(Math.min(3,scale+.25))}><PreviewIcon name="plus"/></button></div></PreviewToolbar>
    <div className="fp-pdf-layout">{outline&&<nav className="fp-thumbnails" aria-label={t('页面缩略图')}>{Array.from({length:pdf.numPages},(_,i)=><Thumbnail key={i+1} pdf={pdf} page={i+1} current={page===i+1} onSelect={()=>go(i+1)}/>)}</nav>}<div className="fp-pdf-viewport" data-preview-page={page}><div ref={stage} className={`fp-pdf-stage ${canPan?'can-pan':''}`} tabIndex={0} onPointerDown={event=>{if(!canPan||event.button!==0)return;event.preventDefault();drag.current={x:event.clientX,y:event.clientY,left:event.currentTarget.scrollLeft,top:event.currentTarget.scrollTop};event.currentTarget.setPointerCapture(event.pointerId);}} onPointerMove={event=>{const start=drag.current;if(start){event.currentTarget.scrollLeft=start.left-(event.clientX-start.x);event.currentTarget.scrollTop=start.top-(event.clientY-start.y);}}} onPointerUp={()=>{drag.current=undefined;}} onPointerCancel={()=>{drag.current=undefined;}} aria-label={t('{name}，使用左右方向键翻页',{name})} onKeyDown={event=>{if(event.key==='ArrowRight'||event.key==='PageDown'){event.preventDefault();go(page+1);}if(event.key==='ArrowLeft'||event.key==='PageUp'){event.preventDefault();go(page-1);}}}><div className="fp-pdf-space" style={{minWidth:width+padding*2,minHeight:Math.max(room.height,pageSize.height*scale)+padding*2}}><PdfPage pdf={pdf} page={page} width={width} aspectRatio={pageSize.width/pageSize.height}/></div></div></div></div>
    </>;
}
