...
export function RunMessage({messages,isLast=true,run,stream,latest,canContinue,onContinue,onSettings,model}:{messages:ChatMessage[];allMessages?:ChatMessage[];isLast?:boolean;run?:RunRecord;stream?:Reply;latest:boolean;canContinue:boolean;waiting?:'host_permission'|'vm_takeover';reviewing?:boolean;onContinue:()=>void;onSettings:(tab:'model'|'computer'|'mcp')=>void;onScreen?:(url:string)=>void;model?:ModelConfig}){
  ...
  return <div className="run-message" data-run-id={run?.id||messages[0]?.runId} data-run-segment={messages[0]?.id} data-run-terminal={isLast}>
    {notes.map(message=><Message key={message.id} message={message} allowPins={!run?.groupOrigin}/>)}
    {showError&&<div className="run-notice" role="status"><span className="run-notice-icon"><Icon name="alert" size={18}/></span><div><strong>{notice.title}</strong><p>{description}</p>{(canContinue||notice.settings)&&<div className="run-notice-actions">{canContinue&&!contextBlocked&&<button onClick={onContinue}>{notice.context?'整理记录并继续':'继续处理'}<Icon name="arrow" size={13}/></button>}{notice.settings&&<button onClick={()=>onSettings(notice.settings!)}>检查设置</button>}</div>}</div></div>}
    {showStopped&&<div className="run-stopped"><p>{view.error?.startsWith('用户拒绝')||view.error?.startsWith('用户取消')?view.error:'已停止，完成的操作和文件仍然保留。'}</p>{canContinue&&<button onClick={onContinue}>继续处理<Icon name="arrow" size={13}/></button>}</div>}
