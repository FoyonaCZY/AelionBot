...
export function RunMessage({messages,allMessages=messages,isLast=true,run,stream,latest,canContinue,waiting,onContinue,onSettings,onScreen}:{messages:ChatMessage[];allMessages?:ChatMessage[];isLast?:boolean;run?:RunRecord;stream?:Reply;latest:boolean;canContinue:boolean;waiting?:'host_permission'|'vm_takeover';onContinue:()=>void;onSettings:(tab:'model'|'computer'|'mcp')=>void;onScreen:(url:string)=>void}){
  ...
  const title=!isLast?'执行记录':running?(waiting==='host_permission'?'等待你的本机操作许可':waiting==='vm_takeover'?'等待人工接管':current?toolOperation(current.tool).active:view.tools.length?'正在整理结果':'正在处理'):failed?(latest?'执行已暂停':notice.title):cancelled?(run?.inputUpdated?'已接收新的输入':run?.groupUpdated?'已接收新的群消息':'已停止'):'执行完成';
  ...
  return <div className="run-message" data-run-id={run?.id||messages[0]?.runId} data-run-segment={messages[0]?.id} data-run-terminal={isLast}>
    ...
    {hasActivity&&<details className={`run-activity ${running?'is-running':''} ${failed?'is-failed':''}`} open={open} onToggle={event=>setOpen(event.currentTarget.open)}>
      <summary><StatusMark state={waiting?'cancelled':running?'running':failed||segmentFailed?'failed':cancelled?'cancelled':'done'}/><span className="activity-heading" role={running?'status':undefined} aria-live={running?'polite':undefined}>{title}</span><span className="activity-meta">{view.tools.length>0&&`${view.tools.length} 个步骤`}{duration&&<span>{duration}</span>}</span><Icon name="down" size={13}/></summary>
