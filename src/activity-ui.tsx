...
export function RunMessage({messages,isLast=true,run,stream,latest,canContinue,onContinue,onSettings,onReply,model}:{messages:ChatMessage[];allMessages?:ChatMessage[];isLast?:boolean;run?:RunRecord;stream?:Reply;latest:boolean;canContinue:boolean;waiting?:'host_permission'|'vm_takeover';reviewing?:boolean;onContinue:()=>void;onSettings:(tab:'model'|'computer'|'mcp')=>void;onScreen?:(url:string)=>void;onReply?:(message:ChatMessage)=>void;model?:ModelConfig}){
