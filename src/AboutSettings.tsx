...
const labels={idle:'',checking:'正在检查更新…',current:'已是最新版本',available:'有新版本可用',downloading:'正在下载更新…',cancelling:'正在取消下载…',downloaded:'更新已下载',installing:'正在准备安装更新…',error:'更新未完成',unsupported:'开发模式下不安装更新，请使用 Windows 发行版。'};
  ...
  return <SettingsSection title="应用"><div className="about-app">
    ...
    {update&&<div className="about-update-content">
      ...
      <div className="about-update-actions"><button className="text-button" onClick={()=>void act(()=>window.aelion.openUpdateRelease())} disabled={pending||phase==='installing'}>查看 GitHub Release</button>
        {['available','error'].includes(phase)&&update.latestVersion&&<button className="primary-button" disabled={pending} onClick={()=>void act(()=>window.aelion.downloadUpdate())}>{phase==='error'?'重试下载':'下载更新'}</button>}
