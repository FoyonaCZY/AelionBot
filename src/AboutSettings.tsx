...
export function AboutSettings({update,onNotify}:{update?:UpdateState;onNotify:(text:string)=>void}){
  ...
  return <><SettingsSection title="应用"><div className="about-app">
    <div className="about-update-heading"><div className="settings-about-brand"><Avatar bot={{name:'AelionBot',color:'#9a7ccb'}} size={58}/><div><span className="brand">Aelion<span>Bot</span></span><p className="about-version">版本 {update?.currentVersion||version}</p></div></div><button className="secondary-button" disabled={pending||working||phase==='unsupported'} onClick={check}>{phase==='checking'?'检查中…':'检查更新'}</button></div>
