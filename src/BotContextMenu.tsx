...
export function BotContextMenu({anchor,name,canDelete,onEdit,onDelete,onClose,onPrivateChats}:{anchor:BotMenuAnchor;name:string;canDelete:boolean;onEdit:()=>void;onDelete:()=>void;onClose:()=>void;onPrivateChats?:()=>void}){
  ...
  return <div ref={menu} role="menu" aria-label={`${name} 的菜单`} className="bot-context-menu" style={{left:anchor.x,top:anchor.y}} onContextMenu={event=>event.preventDefault()} onKeyDown={event=>{
    ...
    <button role="menuitem" onClick={onEdit}><Icon name="edit" size={16}/>编辑</button>
    {onPrivateChats&&<button role="menuitem" onClick={onPrivateChats}><Icon name="message" size={16}/>私聊记录</button>}
    <div className="bot-menu-separator" role="separator"/>
    <button role="menuitem" className="destructive" disabled={!canDelete} title={canDelete?undefined:'请先停止这个 Bot 的任务'} onClick={onDelete}><Icon name="trash" size={16}/>删除</button>
  </div>;
}
