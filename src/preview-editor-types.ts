...
export interface PreviewViewport{width:number;height:number;scrollX:number;scrollY:number;documentWidth:number;documentHeight:number;}
export interface PreviewAnnotation{sourceWidth?:number;sourceHeight?:number;elementLabel?:string;elementText?:string;id:string;type:AnnotationTool;x:number;y:number;w?:number;h?:number;end?:AnnotationPoint;points?:AnnotationPoint[];text?:string;color:string;page?:number;selector?:string;designId?:string;}
...
export interface PreviewEditorState{selectedAnnotationId?:string;annotationCanUndo?:boolean;annotationCanRedo?:boolean;viewport?:PreviewViewport;mode:PreviewMode;tool:AnnotationTool;selected?:PreviewElement;dirty:boolean;canUndo:boolean;canRedo:boolean;annotations:PreviewAnnotation[];revision:number;error?:string;}
export type EditorCommand=(
 ...
 |{type:'feedback-state'}
 |{type:'annotation-select';id?:string}|{type:'annotation-undo'|'annotation-redo'}
