...
function AppContent(){
  ...
  return <PreviewScopeProvider scope={group?{kind:'group',id:group.id}:bot?{kind:'bot',id:bot.id}:undefined}><BotAvatarProvider bots={state.bots}><div className="app-shell" data-platform={state.platform} data-page={page} data-bot-type={!group?bot?.type:undefined}>
    ...
    <main className="conversation">{(previewWorkbench?.info?.docked||previewWorkbench?.info?.studio)&&<PreviewBotSwitcher bots={state.bots} groups={state.groups?.rooms||[]} value={group?'group:'+group.id:'bot:'+bot?.id} onChange={value=>previewWorkbench.navigate(()=>{const [kind,id]=value.split(':');setPage('chat');setSelectedGroup(kind==='group'?id:'');if(kind==='bot')setSelected(id);setFiles([]);})} onSettings={()=>openSettings()} onPlugins={()=>previewWorkbench.navigate(()=>openPlugins())} onNew={()=>previewWorkbench.navigate(openNewBot)}/>}
