...
export function PermissionModePicker({scope,mode='ask'}:{scope:AttachmentScope;mode?:HostPermissionMode}){
  ...
  return <div className={`permission-mode permission-mode-${current.id}`}>
    ...
    <Select aria-label="本机权限模式" title={`当前会话 · ${current.description}`} value={current.id} disabled={pending} onChange={event=>void change(event.target.value as HostPermissionMode)}>{HOST_PERMISSION_MODES.map(item=><option key={item.id} value={item.id}>{item.label}</option>)}</Select>
