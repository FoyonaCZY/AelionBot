...
export function computerDesktopReady(vm:VmState){return vm.status==='ready'&&!vm.operationPending&&Boolean(vm.appsReady)&&!vm.maintenance&&!vm.needsReboot;}
export function computerNeedsAppUpdate(vm:VmState){return !vm.operationPending&&vm.appsReady===false&&(vm.status==='stopped'||vm.status==='ready')&&!vm.maintenance&&!vm.needsReboot&&!vm.lastError;}
export function computerSetupDismissalKey(dataDir:string,vm:VmState){return vm.status==='unprepared'?`aelion-computer-setup:${dataDir}`:`aelion-computer-setup:${dataDir}:ws-${WORKSTATION_VERSION}`;}
export function shouldOfferComputerSetup(vm:VmState,dismissed:boolean){return !dismissed&&(vm.status==='unprepared'||computerNeedsAppUpdate(vm));}
export function computerSetupState(vm:VmState){
  ...
  if(vm.operationPending&&!vm.maintenance)return {kind:'starting',title:'正在启动工作电脑',step:1,working:true} as const;
  ...
  if(vm.status==='stopped'&&vm.appsReady===false)return {kind:'upgrade',title:'更新工作环境',step:2,working:false} as const;
...
export function computerSetupActions(vm:VmState):Array<'start'|'restart'|'repair-tools'>{
  ...
  if(view.kind==='upgrade')return ['start','repair-tools'];
...
export function computerSetupActionLabel(vm:VmState){
  ...
  return kind==='restart'?'重启并完成初始化':kind==='error'?'重试':kind==='stopped'?'启动工作电脑':kind==='incomplete'?'继续初始化':kind==='upgrade'?'更新工作环境':'开始初始化';
}
