...
import {Icon} from './ui';
const designLabels:Record<string,[string,string]>={design_tasks:['查看设计任务','Review design tasks'],design_start:['创建设计任务','Create design task'],design_use:['读取任务状态','Read task state'],design_system:['选择设计系统','Select a design system'],design_spec:['保存设计约定','Save design direction'],design_resource:['读取设计资料','Read design resources'],design_check:['检查实际画面','Inspect rendered output'],design_publish:['整理交付文件','Prepare deliverables'],design_deck:['生成演示文稿','Build the deck'],design_file_create:['写入设计文件','Write a design file'],design_image:['生成插图','Generate an illustration'],design_export_pdf:['导出 PDF','Export PDF'],design_plugin:['读取设计插件','Read a design plugin']};
