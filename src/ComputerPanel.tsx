...
export function ComputerPanel({vm,ready,bot,onOpen,onSetup,onSettings,children}:{vm:VmState;ready:boolean;bot?:Pick<Bot,'id'|'name'|'color'|'avatarStyle'>;onOpen:()=>void;onSetup:()=>void;onSettings:()=>void;children?:ReactNode}){
  ...
  return <section className={`computer-panel ${ready?'is-online':'is-offline'}`} aria-label="工作电脑">
    {ready?<div className="computer-panel-screen" role="button" tabIndex={0} aria-label="全屏查看工作电脑" title="全屏查看工作电脑" style={{aspectRatio:aspect}} onClick={onOpen} onKeyDown={event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();onOpen();}}}>
    ...
    <button type="button" className="computer-panel-settings" aria-label="电脑设置" title="电脑设置" onClick={onSettings}><Icon name="settings" size={17}/></button>
  </section>;
}
