...
function ProviderEditor({provider,disabled,onClose,onSaved}:{provider?:ModelProvider;disabled:boolean;onClose:()=>void;onSaved:(provider:ModelProvider)=>void}){
  ...
  const [parameters,setParameters]=useState<ModelParameters>({protocol:provider?(provider.protocol||'chat'):'responses',responsesTransport:provider?.responsesTransport,hostedWebSearch:provider?.hostedWebSearch,hostedImageGeneration:provider?.hostedImageGeneration,temperature:provider?.temperature,thinkingBudget:provider?.thinkingBudget,fallbackModel:provider?.fallbackModel});
  ...
  return <form ref={form} className="provider-editor" onSubmit={event=>{event.preventDefault();void save();}}>
    <h4>{provider?t('编辑 Provider'):t('添加 Provider')}</h4><div className="settings-card">
      ...
      {parameters.protocol==='responses'&&<label className="settings-row"><span>{t('网页搜索')}</span><label className="provider-hosted-flag"><input type="checkbox" checked={Boolean(parameters.hostedWebSearch)} disabled={disabled||pending} onChange={event=>setParameters({...parameters,hostedWebSearch:event.target.checked||undefined})}/>{t('使用服务端内置搜索，不再提供应用内 web_search')}</label></label>}
      {parameters.protocol==='responses'&&<label className="settings-row"><span>{t('图片生成')}</span><label className="provider-hosted-flag"><input type="checkbox" checked={Boolean(parameters.hostedImageGeneration)} disabled={disabled||pending} onChange={event=>setParameters({...parameters,hostedImageGeneration:event.target.checked||undefined})}/>{t('使用服务端内置生图，由 Responses 直接出图')}</label></label>}
