...
function ProviderEditor({provider,disabled,onClose,onSaved}:{provider?:ModelProvider;disabled:boolean;onClose:()=>void;onSaved:(provider:ModelProvider)=>void}){
  ...
  return <form ref={form} className="provider-editor" onSubmit={event=>{event.preventDefault();void save();}}>
    <h4>{provider?'编辑 Provider':'添加 Provider'}</h4><div className="settings-card">
      ...
      {parameters.protocol==='responses'&&<label className="settings-row"><span>连接方式</span><Select aria-label="Responses 连接方式" value={parameters.responsesTransport||'auto'} disabled={disabled||pending} onChange={e=>setParameters({...parameters,responsesTransport:e.target.value as ModelParameters['responsesTransport']})}><option value="auto">自动</option><option value="http">HTTP</option><option value="websocket">WebSocket（服务端支持时）</option></Select></label>}
