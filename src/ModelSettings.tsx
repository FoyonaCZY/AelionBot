...
function ProviderEditor({provider,disabled,onClose,onSaved}:{provider?:ModelProvider;disabled:boolean;onClose:()=>void;onSaved:(provider:ModelProvider)=>void}){
  ...
  return <form ref={form} className="provider-editor" onSubmit={event=>{event.preventDefault();void save();}}>
    <h4>{provider?t('编辑 Provider'):t('添加 Provider')}</h4><div className="settings-card">
      ...
      {parameters.protocol==='responses'&&<label className="settings-row provider-hosted-row"><span>{t('服务端搜索')}</span><input type="checkbox" checked={Boolean(parameters.hostedWebSearch)} disabled={disabled||pending} onChange={event=>setParameters({...parameters,hostedWebSearch:event.target.checked})} aria-label={t('使用服务端内置搜索，不再提供应用内 web_search')}/></label>}
      {parameters.protocol==='responses'&&<label className="settings-row provider-hosted-row"><span>{t('服务端生图')}</span><input type="checkbox" checked={Boolean(parameters.hostedImageGeneration)} disabled={disabled||pending} onChange={event=>setParameters({...parameters,hostedImageGeneration:event.target.checked})} aria-label={t('使用服务端内置生图，由 Responses 直接出图')}/></label>}
...
function ProviderModelCatalog({provider,disabled,onChange}:{provider:ModelProvider;disabled:boolean;onChange:(provider:ModelProvider)=>void}){
  const {t}=useI18n();
  const [customId,setCustomId]=useState(''),thinking=['anthropic','gemini'].includes(provider.protocol||'chat');
  ...
  return <div className="provider-model-catalog">
    <h4>{t('模型目录')}</h4>
    <p className="provider-model-hint">{t('为每个模型预先设置上下文和推理。选择模型时会使用这些值。')}</p>
