...
export function ModelSettings({state,onNotify}:{state:Snapshot;onNotify:(text:string)=>void}){
  ...
  return <>
    ...
    <SettingsSection title="Providers">
      ...
      {editing&&(editing==='new'||editor)&&<ProviderEditor key={editing} provider={editor} disabled={editingBusy} onClose={()=>setEditing(undefined)} onSaved={provider=>{setEditing(provider.id);onNotify(provider.modelsError?'Provider 已保存，模型列表拉取失败':'Provider 已保存');}}/>}
    </SettingsSection>
  </>;
}
...
function ProviderEditor({provider,disabled,onClose,onSaved}:{provider?:ModelProvider;disabled:boolean;onClose:()=>void;onSaved:(provider:ModelProvider)=>void}){
  const [parameters,setParameters]=useState<ModelParameters>({protocol:provider?(provider.protocol||'chat'):'responses',temperature:provider?.temperature,reasoningEffort:provider?.reasoningEffort,thinkingBudget:provider?.thinkingBudget,fallbackModel:provider?.fallbackModel});
  ...
  return <form className="provider-editor" onSubmit={event=>{event.preventDefault();void save();}}>
    ...
    <div className="settings-actions"><button className="secondary-button" type="button" disabled={pending} onClick={onClose}>关闭</button><button className="primary-button" disabled={disabled||pending||!name.trim()||!baseUrl.trim()}>{pending?'保存并拉取中…':'保存并拉取模型'}</button></div>
  </form>;
}
