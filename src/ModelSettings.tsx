...
function ProviderEditor({provider,disabled,onClose,onSaved}:{provider?:ModelProvider;disabled:boolean;onClose:()=>void;onSaved:(provider:ModelProvider)=>void}){
  ...
  const [parameters,setParameters]=useState<ModelParameters>({protocol:provider?(provider.protocol||'chat'):'responses',responsesTransport:provider?.responsesTransport,temperature:provider?.temperature,thinkingBudget:provider?.thinkingBudget,fallbackModel:provider?.fallbackModel});
  ...
  return <form ref={form} className="provider-editor" onSubmit={event=>{event.preventDefault();void save();}}>
    ...
    <div className="settings-actions"><button className="secondary-button" type="button" disabled={pending} onClick={onClose}>{t('关闭')}</button><button className="primary-button" disabled={disabled||pending||!name.trim()||!baseUrl.trim()}>{pending?t('保存并拉取中…'):provider?t('保存修改并拉取模型'):t('添加并拉取模型')}</button></div>
    {provider&&<ProviderModelCatalog provider={provider} disabled={disabled||pending} onChange={onSaved}/>}
  </form>;
}
...
function ProviderModelCatalog({provider,disabled,onChange}:{provider:ModelProvider;disabled:boolean;onChange:(provider:ModelProvider)=>void}){
  const {t}=useI18n();
  const [customId,setCustomId]=useState(''),responses=(provider.protocol||'chat')==='responses',thinking=['anthropic','gemini'].includes(provider.protocol||'chat');
  ...
  return <div className="provider-model-catalog">
    <h4>{t('模型目录')}</h4>
    <p className="provider-model-hint">{t('为每个模型预先设置上下文、推理和 Responses 内建工具。选择模型时会使用这些值。')}</p>
    {provider.models.map(model=><details key={model.id} className="provider-model-card"><summary>{model.id}</summary>
      ...
      <label className="provider-hosted-flag"><input type="checkbox" checked={Boolean(model.supportsImages)} disabled={disabled} onChange={event=>void save({...model,supportsImages:event.target.checked})}/><span>{t('支持图片输入')}</span></label>
      {responses&&<label className="provider-hosted-flag"><input type="checkbox" checked={Boolean(model.hostedWebSearch)} disabled={disabled} onChange={event=>void save({...model,hostedWebSearch:event.target.checked})}/><span>{t('使用服务端内置搜索，不再提供应用内 web_search')}</span></label>}
      {responses&&<label className="provider-hosted-flag"><input type="checkbox" checked={Boolean(model.hostedImageGeneration)} disabled={disabled} onChange={event=>void save({...model,hostedImageGeneration:event.target.checked})}/><span>{t('使用服务端内置生图，由 Responses 直接出图')}</span></label>}
    </details>)}
