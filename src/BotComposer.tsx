...
export function BotComposer({bot,bots,draft=empty,running,onChange,onSend,onStop,attachmentScope,workspaceDir,workspaceInherited,permissionMode,contextOverview,contextCapacity}:{contextOverview?:ContextOverview;contextCapacity?:number;bot:Pick<Bot,'id'|'name'>;bots:Bot[];draft?:ComposerDraft;running:boolean;attachmentScope?:AttachmentScope;workspaceDir?:string;workspaceInherited?:boolean;permissionMode?:HostPermissionMode;onChange:(draft:ComposerDraft)=>void;onSend:()=>void;onStop:()=>void}){
  const {t}=useI18n();
  ...
  useEffect(()=>{setFileOptions([]);setFileSearchError('');if(!query){setFilesLoading(false);return;}let current=true;setFilesLoading(true);const timer=setTimeout(()=>{window.aelion.searchMentionFiles({scope,query:query.text}).then(result=>{if(current)setFileOptions(result.files);}).catch(()=>{if(current)setFileSearchError(t('无法搜索文件，请确认工作目录可用。'));}).finally(()=>{if(current)setFilesLoading(false);});},180);return()=>{current=false;clearTimeout(timer);};},[query?.text,query?.start,scope.kind,scope.id,workspaceDir]);
