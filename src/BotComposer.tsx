...
export function BotComposer({bot,bots,draft=empty,running,onChange,onSend,onStop,attachmentScope,workspaceDir,workspaceInherited,permissionMode,contextOverview,contextCapacity,contextControl,fixedDesignWorkspace=false,designSessionId,placeholder,extraTools}:{placeholder?:string;extraTools?:ReactNode;designSessionId?:string;fixedDesignWorkspace?:boolean;contextControl?:ReactNode;contextOverview?:ContextOverview;contextCapacity?:number;bot:Pick<Bot,'id'|'name'>;bots:Bot[];draft?:ComposerDraft;running:boolean;attachmentScope?:AttachmentScope;workspaceDir?:string;workspaceInherited?:boolean;permissionMode?:HostPermissionMode;onChange:(draft:ComposerDraft)=>void;onSend:()=>void;onStop:()=>void}){
  ...
  const ingest=(operation:(isCurrent:()=>boolean)=>Promise<Attachment[]>)=>{
    ...
    uploadChain.current=uploadChain.current.then(async()=>{if(!isCurrent())return;const added=await operation(isCurrent);if(!isCurrent())return;const latest=draftRef.current,files=[...new Map([...(latest.attachments||[]),...added].map(file=>[file.id,file])).values()];if(files.length>ATTACHMENT_LIMITS.count)throw Error(t('每条消息最多附加 10 个文件'));if(files.reduce((total,file)=>total+file.size,0)>ATTACHMENT_LIMITS.totalBytes)throw Error(t('附件总大小不能超过 100 MB'));update({...latest,attachments:files});}).catch(error=>{if(isCurrent()){const text=(error as Error).message.replace(/^Error invoking remote method '[^']+': Error: /,'');setUploadError(/could not be found|not readable|ENOENT|EISDIR|NotFoundError/.test(text)?t('无法读取选中的文件，请确认文件仍在原位置后重新选择。'):text);}}).finally(()=>{if(isCurrent()){uploadCount.current--;setUploading(uploadCount.current);}});
  };
  ...
  const importFiles=(files:File[])=>{
    if(files.length>10){setUploadError(t('一次最多拖入 10 个文件或文件夹'));return;}
    ingest(async isCurrent=>{const prepared=await window.aelion.prepareAttachmentDrop({scope,files});if(!isCurrent())return [];const virtual=prepared.virtualIndexes.map(index=>files[index]);if(virtual.some(file=>file.size>ATTACHMENT_LIMITS.fileBytes))throw Error(t('单个附件不能超过 25 MB'));if(virtual.reduce((sum,file)=>sum+file.size,0)>ATTACHMENT_LIMITS.totalBytes)throw Error(t('附件总大小不能超过 100 MB'));const uploaded=virtual.length?await window.aelion.importAttachments({scope,files:await Promise.all(virtual.map(async file=>({name:file.name||t('粘贴图片.png'),bytes:new Uint8Array(await file.arrayBuffer())})))}):[];return [...uploaded,...await receiveEntries(prepared.entries,isCurrent)];});
  };
  ...
  const botOptions=query?bots.filter(item=>item.id!==bot.id&&`${item.name} ${item.role}`.toLowerCase().includes(query.text.toLowerCase())).slice(0,10):[];
  const [fileOptions,setFileOptions]=useState<MentionFile[]>([]),[fileSearchError,setFileSearchError]=useState(''),[filesLoading,setFilesLoading]=useState(false);
  useEffect(()=>{setFileOptions([]);setFileSearchError('');if(!query){setFilesLoading(false);return;}let current=true;setFilesLoading(true);const timer=setTimeout(()=>{window.aelion.searchMentionFiles({scope,query:query.text,designSessionId}).then(result=>{if(current)setFileOptions(result.files);}).catch(()=>{if(current)setFileSearchError(t('无法搜索文件，请确认工作目录可用。'));}).finally(()=>{if(current)setFilesLoading(false);});},180);return()=>{current=false;clearTimeout(timer);};},[query?.text,query?.start,scope.kind,scope.id,workspaceDir,designSessionId]);
  const options:Array<{kind:'bot';bot:Bot}|{kind:'file';file:MentionFile}>=[...botOptions.map(bot=>({kind:'bot' as const,bot})),...fileOptions.map(file=>({kind:'file' as const,file}))];
  const refresh=()=>{
  ...
  const chooseOption=(option:typeof options[number])=>{
    if(option.kind==='bot'){choose(option.bot);return;}
    const root=editor.current;if(!root||!query)return;root.focus({preventScroll:true});const range=document.createRange();range.setStart(...position(root,query.start));range.setEnd(...position(root,query.end));range.deleteContents();
    const node=document.createTextNode(fileMentionText(option.file)+' ');range.insertNode(node);range.setStartAfter(node);range.collapse(true);const selection=getSelection();selection?.removeAllRanges();selection?.addRange(range);refresh();setQuery(undefined);
  };
  ...
  return <div className={`composer mention-composer ${dragging?'is-dragging':''}`} onDragOver={event=>{if(event.dataTransfer.types.includes('Files')){event.preventDefault();event.dataTransfer.dropEffect='copy';setDragging(true);}}} onDragLeave={event=>{if(!event.currentTarget.contains(event.relatedTarget as Node|null))setDragging(false);}} onDrop={event=>{event.preventDefault();setDragging(false);const files=Array.from(event.dataTransfer.files);if(files.length)importFiles(files);}}>
    ...
    {directories.length>0&&<div className="composer-directory-choices">{directories.map(entry=><div className="composer-directory-choice" key={entry.id}><Icon name="folder" size={17}/><strong title={entry.name}>{entry.name}</strong><button type="button" disabled={uploading>0} onClick={()=>useDirectory(entry,'workspace')}>{t('设为工作目录')}</button><button type="button" disabled={uploading>0} onClick={()=>useDirectory(entry,'attach')}>{t('打包为附件')}</button><button type="button" className="icon-button" aria-label={t('移除文件夹 {name}',{name:entry.name})} onClick={()=>setDirectories(current=>current.filter(item=>item.id!==entry.id))}><Icon name="close" size={14}/></button></div>)}</div>}
    {uploading>0&&<div className="composer-uploading" role="status">{t('正在添加附件…')}</div>}{uploadError&&<div className="composer-attachment-error" role="alert"><span>{uploadError}</span><button type="button" aria-label={t('关闭附件错误')} onClick={()=>setUploadError('')}><Icon name="close" size={14}/></button></div>}
    {commands.length>0&&<div className="command-picker" role="listbox" id={id+'-commands'} aria-label={t('选择计划或目标')}>{commands.map((command,index)=><button type="button" key={command.name} id={id+'-command-'+index} role="option" aria-selected={index===commandActive} className={index===commandActive?'active':''} onMouseDown={event=>event.preventDefault()} onClick={()=>chooseCommand(command.name)}><span className={`composer-menu-icon composer-menu-${command.name}`}><CompanionGlyph kind={command.name}/></span><span className="command-copy"><strong>{t(command.label)}</strong><small>{t(command.description)}</small></span><code>/{command.name}</code></button>)}</div>}
    {query&&<div ref={list} className="mention-picker" role="listbox" id={id} aria-label={t('选择 Bot 或文件')}>{options.map((option,index)=><button key={option.kind==='bot'?'bot:'+option.bot.id:'file:'+option.file.path} title={option.kind==='file'?option.file.path:undefined} id={`${id}-${index}`} role="option" aria-selected={index===active} className={index===active?'active':''} onMouseDown={event=>event.preventDefault()} onClick={()=>chooseOption(option)}>{option.kind==='bot'?<Avatar bot={option.bot} size={30}/>:<Icon name="file" size={24}/>}<span><strong>{option.kind==='bot'?option.bot.name:option.file.relativePath}</strong><small>{option.kind==='bot'?option.bot.role||'Bot':t('文件引用 · 不复制原文件')}</small></span></button>)}{filesLoading&&<div className="mention-empty" role="status">{t('正在搜索文件…')}</div>}{fileSearchError&&<div className="mention-empty">{fileSearchError}</div>}{!options.length&&!filesLoading&&!fileSearchError&&<div className="mention-empty">{t('没有匹配的 Bot 或文件')}</div>}</div>}
    <div ref={editor} className="composer-editor" role="textbox" aria-label={composerPlaceholder} aria-multiline="true" aria-autocomplete="list" aria-controls={commands.length?id+'-commands':query?id:undefined} aria-expanded={Boolean(query||commands.length)} aria-activedescendant={commands.length?id+'-command-'+commandActive:query&&options.length?`${id}-${Math.min(active,options.length-1)}`:undefined} contentEditable suppressContentEditableWarning data-placeholder={composerPlaceholder} data-empty={!draft.text} spellCheck={false}
      ...
      onKeyDown={event=>{
        ...
        if(query){if(filesLoading&&!options.length&&(event.key==='Enter'||event.key==='Tab')){event.preventDefault();return;}if(event.key==='Escape'){event.preventDefault();event.stopPropagation();setQuery(undefined);return;}if(options.length&&(event.key==='ArrowDown'||event.key==='ArrowUp')){event.preventDefault();setActive(value=>(value+(event.key==='ArrowDown'?1:-1)+options.length)%options.length);return;}if(options.length&&(event.key==='Enter'||event.key==='Tab')){event.preventDefault();chooseOption(options[Math.min(active,options.length-1)]);return;}}
    ...
    <div className="composer-bottom"><ComposerTools fixedDesignWorkspace={fixedDesignWorkspace} contextControl={contextControl} scope={scope} workspaceDir={workspaceDir} workspaceInherited={workspaceInherited} onFolderPicked={()=>editor.current?.focus({preventScroll:true})} onAttach={()=>ingest(async()=>{const files=await window.aelion.pickAttachments(scope);editor.current?.focus({preventScroll:true});return files;})} onCommand={chooseCommand}/>{extraTools}{scope.kind==='bot'&&<><ContextUsageIndicator overview={contextOverview} capacity={contextCapacity}/><PermissionModePicker scope={scope} mode={permissionMode}/></>}{running&&hasContent&&<button className="icon-button" aria-label={t('停止任务')} onClick={onStop}><span className="stop-square"/></button>}{running&&!hasContent?<button className="send-button" aria-label={t('停止任务')} onClick={onStop}><span className="stop-square"/></button>:<button className="send-button" aria-label={t('发送消息')} disabled={!hasContent||uploading>0||Boolean(workCommand(draft.text)&&!workCommand(draft.text)!.objective)} onClick={()=>sendRef.current()}><Icon name="send"/></button>}</div>
  </div>;
}
