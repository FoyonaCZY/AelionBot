...
export function BotComposer({bot,bots,draft=empty,running,onChange,onSend,onStop,attachmentScope,workspaceDir,permissionMode,contextOverview,contextCapacity}:{contextOverview?:ContextOverview;contextCapacity?:number;bot:Pick<Bot,'id'|'name'>;bots:Bot[];draft?:ComposerDraft;running:boolean;attachmentScope?:AttachmentScope;workspaceDir?:string;permissionMode?:HostPermissionMode;onChange:(draft:ComposerDraft)=>void;onSend:()=>void;onStop:()=>void}){
  ...
  return <div className={`composer mention-composer ${dragging?'is-dragging':''}`} onDragOver={event=>{if(event.dataTransfer.types.includes('Files')){event.preventDefault();event.dataTransfer.dropEffect='copy';setDragging(true);}}} onDragLeave={event=>{if(!event.currentTarget.contains(event.relatedTarget as Node|null))setDragging(false);}} onDrop={event=>{event.preventDefault();setDragging(false);const files=Array.from(event.dataTransfer.files);if(files.length)importFiles(files);}}>
    ...
    {query&&<div ref={list} className="mention-picker" role="listbox" id={id} aria-label={t('选择要联系的 Bot')}>{options.length?options.map((item,index)=><button key={item.id} id={`${id}-${index}`} role="option" aria-selected={index===active} className={index===active?'active':''} onMouseDown={event=>event.preventDefault()} onClick={()=>choose(item)}><Avatar bot={item} size={30}/><span><strong>{item.name}</strong><small>{item.role||'Bot'}{bots.filter(bot=>bot.name===item.name).length>1?` · ${item.id.slice(0,6)}`:''}</small></span></button>):<div className="mention-empty">{t('没有匹配的 Bot')}</div>}</div>}
