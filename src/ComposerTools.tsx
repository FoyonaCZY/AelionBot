...
export function ComposerTools({scope,workspaceDir,workspaceInherited=false,contextControl,onAttach,onCommand,onFolderPicked,fixedDesignWorkspace=false}:{fixedDesignWorkspace?:boolean;scope:AttachmentScope;contextControl?:ReactNode;workspaceDir?:string;workspaceInherited?:boolean;onAttach:()=>void;onCommand:(mode:WorkMode)=>void;onFolderPicked:()=>void}){
  const {t}=useI18n();
  ...
  return <div ref={root} className="composer-tools" onKeyDown={event=>{
    ...
    {open&&<div className="composer-add-menu" role="menu" aria-label={t('添加到会话')}>
      ...
      {!fixedDesignWorkspace&&<><button type="button" role="menuitem" onClick={()=>void folder()}><span className="composer-menu-icon"><Icon name="folder"/></span><span>{workspaceDir?t('更换工作目录'):t('选择工作目录')}</span></button>
