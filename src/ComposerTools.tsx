...
export function ComposerTools({scope,workspaceDir,onAttach,onCommand,onFolderPicked}:{scope:AttachmentScope;workspaceDir?:string;onAttach:()=>void;onCommand:(mode:WorkMode)=>void;onFolderPicked:()=>void}){
  ...
  return <div ref={root} className="composer-tools" onKeyDown={event=>{
    ...
    <button ref={button} type="button" className="icon-button composer-plus" aria-label="添加附件或工作目录" aria-haspopup="menu" aria-expanded={open} disabled={pending} onClick={()=>setOpen(!open)}><Icon name="plus" size={23}/></button>
    {workspaceDir&&<div className="composer-workspace" title={workspaceDir}><button type="button" disabled={pending} onClick={()=>void folder()} aria-label={`工作目录：${workspaceDir}`}><Icon name="folder" size={15}/><span>{workspaceDir.split(/[\\/]/).filter(Boolean).at(-1)||workspaceDir}</span></button><button type="button" className="workspace-remove" aria-label="清除会话工作目录" disabled={pending} onClick={()=>void folder(true)}><Icon name="close" size={13}/></button></div>}
    {open&&<div className="composer-add-menu" role="menu" aria-label="添加到会话">
      <button type="button" role="menuitem" onClick={()=>{close();onAttach();}}><span className="composer-menu-icon"><AttachmentIcon/></span><span>上传附件</span></button>
      <button type="button" role="menuitem" onClick={()=>void folder()}><span className="composer-menu-icon"><Icon name="folder"/></span><span>{workspaceDir?'更换工作目录':'选择工作目录'}</span></button>
      <div className="composer-menu-divider"/>
      <button type="button" role="menuitem" onClick={()=>{close();onCommand('plan');}}><span className="composer-menu-icon composer-menu-plan"><CompanionGlyph kind="plan"/></span><span>制定计划</span><kbd>/plan</kbd></button>
      <button type="button" role="menuitem" onClick={()=>{close();onCommand('goal');}}><span className="composer-menu-icon composer-menu-goal"><CompanionGlyph kind="goal"/></span><span>执行目标</span><kbd>/goal</kbd></button>
    </div>}
    {pending&&<span className="workspace-pending" role="status">正在选择目录…</span>}
    {error&&<div className="composer-tools-error" role="alert">{error}<button aria-label="关闭目录错误" onClick={()=>setError('')}><Icon name="close" size={14}/></button></div>}
  </div>;
}
