...
export interface PreviewFeedbackInput {
 view?:import('./preview-editor-types').PreviewViewport;
...
export function previewFeedbackMessage(input:Pick<PreviewFeedbackInput,'text'|'file'|'language'|'annotations'|'edits'|'view'>){
 if(typeof input.text!=='string'||!input.text.trim()||input.text.length>32000||!input.file||typeof input.file.name!=='string'||!input.file.name.trim()||input.file.name.length>300||input.file.path!==undefined&&(typeof input.file.path!=='string'||input.file.path.length>1500)||input.file.attachmentId!==undefined&&(typeof input.file.attachmentId!=='string'||input.file.attachmentId.length>100)||input.file.url!==undefined&&(typeof input.file.url!=='string'||input.file.url.length>4096)||input.file.page!==undefined&&(!Number.isInteger(input.file.page)||input.file.page<1||input.file.page>100000))throw Error('修改意见或文件信息无效');
 if(input.view){const view=input.view;for(const key of ['width','height','scrollX','scrollY','documentWidth','documentHeight'] as const)if(!Number.isFinite(view[key])||Math.abs(view[key])>10000000)throw Error('画布坐标信息无效');if(view.width<1||view.height<1||view.documentWidth<1||view.documentHeight<1)throw Error('画布尺寸无效');}
 const clean=(text:string)=>text.replace(/[\r\n\u0000-\u001f]/g,' ');
 const en=input.language==='en',tw=input.language==='zh-TW';
 const message=(en?'Preview: ':tw?'預覽：':'预览：')+clean(input.file.name)+(input.file.page?(en?' · Page ':' · 第 ')+input.file.page+(en?'':' 页'):'')+'\n'+input.text.trim()+'\n'+(input.file.path?'\n'+(en?'File: ':'文件：')+clean(input.file.path):'')+(input.file.url?'\nURL: '+clean(feedbackWebUrl(input.file.url)):'')+(input.file.attachmentId?'\n'+(en?'Source attachment: ':tw?'原附件：':'原附件：')+clean(input.file.attachmentId):'')+'\n'+(en?'Screenshot: visible area only.':tw?'截圖僅包含目前可見範圍。':'截图仅包含当前可见范围。')+(input.file.unsaved?(en?' Includes unsaved edits.':tw?'畫面含未儲存的修改。':'画面含未保存的修改。'):'');
 const view=input.view?'\n'+(en?'Canvas (CSS pixels): ':'画布（CSS 像素）：')+`document ${input.view.documentWidth}×${input.view.documentHeight}; viewport ${input.view.width}×${input.view.height}; scroll ${input.view.scrollX},${input.view.scrollY}`:'';
 return message+view+(input.annotations?.length?'\n\n'+(en?'Annotations (normalized to their saved document/content dimensions; screenshot shows only the viewport):':'标注（坐标按记录的整页或内容尺寸归一化；截图只包含可见区域）：')+'\n'+annotationContext(input.annotations):'')+(input.edits?.length?'\n'+(en?'The attached preview-changes JSON records the requested DOM changes. Match them to the project source; do not replace a source file with a runtime page snapshot.':'附带的 preview-changes JSON 记录了请求的 DOM 修改。请映射到项目源码，不要用运行时整页快照覆盖源文件。'):'');
}
