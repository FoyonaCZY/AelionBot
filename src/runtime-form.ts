...
export type RuntimeNumberKey=keyof Omit<RuntimeSettings,'fileCheckpoints'|'wakeAllGroupBots'>;
...
export interface RuntimeDraft {numbers:Record<RuntimeNumberKey,string>;fileCheckpoints:boolean;wakeAllGroupBots:boolean;}
export function runtimeDraft(settings:RuntimeSettings):RuntimeDraft{
  return {numbers:Object.fromEntries(RUNTIME_FIELDS.map(field=>[field.key,String(settings[field.key]/(field.factor||1))])) as RuntimeDraft['numbers'],fileCheckpoints:settings.fileCheckpoints,wakeAllGroupBots:settings.wakeAllGroupBots};
}
...
export function runtimeDraftValues(draft:RuntimeDraft):RuntimeSettings|undefined{
  ...
  const result={...DEFAULT_RUNTIME,fileCheckpoints:draft.fileCheckpoints,wakeAllGroupBots:draft.wakeAllGroupBots};
