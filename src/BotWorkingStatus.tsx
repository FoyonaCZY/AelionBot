...
export function BotWorkingStatus({bot,step,showName=false,onStop,onReview}:{bot:Pick<Bot,'id'|'name'|'color'|'avatarStyle'>;step:LiveBotProgress;showName?:boolean;onStop?:()=>Promise<unknown>;onReview?:()=>void}){
  ...
  const slow=waitingExplanation(step,now),description=slow||step.description,recent=step.recent||[];
  ...
  return <div className="bot-working-status" data-working-bot={bot.id} data-phase={step.phase}>
    ...
    <div className="bot-working-copy">{showName&&<span className="bot-working-name">{bot.name}</span>}
      <div role="status" aria-live="polite" aria-atomic="true"><div className="bot-working-title"><span className="bot-working-label">{slow&&step.waitingOn==='model'&&!step.retry?t('还在等待模型回复'):step.label}</span>{slow&&<span className="bot-working-tag">{t('等待较久')}</span>}</div>{description&&<p className="bot-working-description">{description}</p>}{step.detail&&<p className="bot-working-detail">{step.detail}</p>}</div>
