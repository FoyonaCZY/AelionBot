...
function loopFixture(t:any,kind:'prototype'|'ppt'='prototype'){
 ...
 const files=new DesignerFiles(store,designs);const vm:any={};const artifacts={read:(botId:string,path:string)=>files.read(botId,path),collect:async()=>{}};
 ...
 return {...f,systems,designs,task,files,requests,invocations,vm,make:(complete:any,contextOverride:any=context)=>new DesignerLoop(store,designs,systems,files,{complete} as any,contextOverride as any,shared as any,artifacts as any,attachments as any,interactions as any,()=>{})};
}
