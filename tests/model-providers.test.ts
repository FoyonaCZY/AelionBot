...
test('private replies and group notifications use each recipient own Provider',async t=>{
  ...
  a.requests.length=b.requests.length=0;a.config.respond=b.config.respond=async()=>answer('[群聊静默]');groups=new GroupChats(f.store,runner,()=>{});harness.setGroupGateway(groups);groups.start();const room=groups.create({name:'不同模型的群',botIds:[botA.id,botB.id]});await until(()=>!groups.busy&&!harness.busy&&!f.store.data.groupDeliveries.some(delivery=>groupPending(delivery.status)));assert.equal(a.requests.length,0);assert.equal(b.requests.length,0);
  groups.send({id:room.id,message:'请回复状态'});await until(()=>!groups.busy&&!harness.busy&&!f.store.data.groupDeliveries.some(delivery=>groupPending(delivery.status)));assert.equal(a.requests.length,1);assert.equal(b.requests.length,0);assert.equal(a.requests[0].body.model,'alpha-model');
  const tag='@'+botB.name;groups.send({id:room.id,message:tag+' 也请回复',mentions:[{id:botB.id,name:botB.name,color:botB.color,start:0,end:tag.length}]});await until(()=>!groups.busy&&!harness.busy&&!f.store.data.groupDeliveries.some(delivery=>groupPending(delivery.status)));assert.ok(b.requests.length>0);assert.equal(b.requests[0].body.model,'beta-model');
});
