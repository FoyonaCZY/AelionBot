...
function fake(t:test.TestContext,failCompare=false){const dir=directory(t);writeFileSync(join(dir,'system.qcow2'),Buffer.alloc(1024,7));const calls:string[][]=[];const storage=new VmStorage(dir,'v2',async args=>{calls.push(args);if(args[0]==='info')return JSON.stringify({format:'qcow2'});if(args[0]==='convert'&&args[1]==='--help')return '-b BACKING_FILE';if(args[0]==='convert')writeFileSync(args.at(-1)!,Buffer.alloc(128,7));if(args[0]==='compare'&&failCompare)throw Error('mismatch');return '';});return {dir,storage,calls};}
test('storage defaults are finite, persist, and leave a reserve before protection pauses',t=>{
 const dir=directory(t),s=new VmStorage(dir,'v1',async()=>''),defaults=s.snapshot().settings;assert.deepEqual(defaults,DEFAULT_VM_STORAGE);
