...
test('nonblocking questions permit independent reading and their answers return to the same run',async t=>{
 const f=fixture(t,(messages,index)=>{
  ...
  assert.match(JSON.stringify(messages),/测试/);const reply=messages.find(message=>message.role==='user'&&message.content?.startsWith('用户对会话内问题的回答：'))!;assert.equal(JSON.parse(reply.content!.slice('用户对会话内问题的回答：'.length)).answers.scope,'测试');return {content:'已读取文件，接下来按测试范围处理。',finishReason:'stop',calls:[]};
 ...
 const question=f.interactions.snapshot().find(request=>request.kind==='user_input');assert.ok(question);assert.ok(f.store.data.messages.some(message=>message.tool==='host_file_read'&&message.status==='done'));f.interactions.answer(question.id,{scope:'测试'});await pending;assert.equal(f.store.data.runs[0].status,'completed',f.store.data.runs[0].error||'');assert.equal(f.store.data.messages.filter(message=>message.role==='user').length,2);assert.equal(f.store.data.messages.filter(message=>message.role==='user').at(-1)?.content,'测试');assert.deepEqual(f.store.data.messages.filter(message=>message.role==='user').at(-1)?.questionAnswer?.items.map(item=>item.answer),['测试']);
});
