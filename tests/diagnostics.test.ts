...
test('diagnostic bundles preserve useful failures while excluding chat, file, config and tool content',async t=>{
 ...
 const report=JSON.parse(strFromU8(files['diagnostics.json']));assert.equal(report.runs[0].id,'run');assert.equal(report.runs[0].modelCalls,3);assert.equal(report.runs[0].context.estimatedTokens,40000);assert.equal(report.runs[0].executions[0].error,'ECONNRESET');assert.equal(report.modelUsage[0].usage.inputTokens,1200);assert.equal(report.modelUsage[0].usage.cachedTokens,800);assert.deepEqual(report.modelUsage[0].timing,{preparationMs:10,responseMs:20,firstEventMs:25,firstTextMs:30,totalMs:50});assert.match(report.runs[0].error,/HTTP 503/);
