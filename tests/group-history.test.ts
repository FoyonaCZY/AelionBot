...
function fixture(t:test.TestContext){
  ...
  store.data.groups.push(room);return {dir,store,bot,room,key:groupContextKey(room.id),legacyKey:groupContextKey(room.id,bot.id)};
}
...
test('legacy per-Bot contexts are archived and rebuilt as one shared published transcript',t=>{
  const {dir,store,bot,room,key,legacyKey}=fixture(t),id=randomUUID();
  ...
  store.data.groupContexts[legacyKey]=[{role:'user',content:'旧的重复事件'},{role:'assistant',content:'我已经打过招呼了，但实际没有发出去'},{role:'assistant',content:'未发送的工具过程草稿',tool_calls:[{id:'call-1',type:'function',function:{name:'file_write',arguments:'{}'}}]},{role:'tool',tool_call_id:'call-1',content:'{"saved":true,"path":"/work/proof.csv"}'}];
  store.data.summaries[legacyKey]='旧摘要包含未发出的问候';store.data.contextOffsets[legacyKey]=2;
  const history=groupHistory(store,room.id,bot.id),text=JSON.stringify(history);
  assert.ok(!text.includes('草稿')&&!text.includes('没有发出去')&&!text.includes('重复事件')&&!text.includes('/work/proof.csv'));assert.equal(history.length,1);assert.equal(history.at(-1)?.groupMessageId,id);
  assert.equal(groupHistory(store,room.id,bot.id).length,1);assert.equal(store.data.contextOffsets[groupContextKey(room.id)],0);assert.equal(store.data.summaries[groupContextKey(room.id)],undefined);
  const backup=join(dir,'group-context-backups',`${room.id}-shared-migration.json`);assert.ok(existsSync(backup));assert.ok(readFileSync(backup,'utf8').includes('实际没有发出去'));
...
test('long group histories compact within the input budget including tool schemas, retain originals and persist independently',async t=>{
  const {store,dir,key,bot}=fixture(t);store.data.model.contextTokens=8000;
  ...
  const result=await prepareGroupContext(store,model,{botId:bot.id,key,runId:'compression-test',system,history,tools,signal:new AbortController().signal});
...
test('interrupted and invalid group summaries never advance the persisted boundary',async t=>{
  const {store,key,bot}=fixture(t);store.data.model.contextTokens=8000;const history=store.data.groupContexts[key]=longHistory(),controller=new AbortController();
  const input={botId:bot.id,key,runId:'interrupted',system:{role:'system' as const,content:'群聊'},history,tools:[],signal:controller.signal};
