...
test('private reply previews stay in their thread and the sender summary streams in the user conversation',async t=>{
  ...
  const f=fixture(t,async(messages,_tools,_signal,onText)=>{const system=messages[0].content||'';if(system.includes(`/work/${other.id}`)){onText?.('私聊回复片段');return received.promise;}if(system.includes('这是先前联络的实际回信')){onText?.('给用户的总结片段');return summary.promise;}if(!sent){sent=true;return tool('bot_send_message',{botId:other.id,message:'当前状态如何？'});}return answer('已联络');});const other=f.store.createBot('乙','协作');
