...
import {AppUpdates,updateError,type UpdateDriver} from '../electron/core/app-updates';
...
function fixture(t:test.TestContext){const driver=new Driver(),events:string[]=[],control={blocked:undefined as string|undefined,prepare:async(_version:string)=>{events.push('prepared');},recover:async()=>{events.push('recovered');}};const service=new AppUpdates(driver,'0.4.0','owner/repo',true,{blockedReason:()=>control.blocked,prepareInstall:version=>control.prepare(version),recoverInstall:()=>control.recover()},()=>events.push(service.snapshot().phase));t.after(()=>service.dispose());return {driver,service,events,control};}
