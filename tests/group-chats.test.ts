...
function fixture(t:test.TestContext,complete:(run:RunRecord,messages:WireMessage[],tools:ToolDefinition[],signal:AbortSignal)=>Completion|Promise<Completion>=()=>silent(),vm?:VmController,withCognition=false){
  ...
  const model={complete:async(messages:WireMessage[],tools:ToolDefinition[],signal:AbortSignal)=>{const id=/\/work\/([a-f0-9-]+)/.exec(messages[0].content||'')?.[1]||store.data.bots.find(bot=>messages[0].content?.startsWith(`你是 ${bot.name}，职责：`))?.id;const run=store.data.runs.find(r=>r.botId===id&&r.status==='running')!;assert.ok(run);try{return await complete(run,messages,tools,signal);}catch(error){if((error as Error).name==='AssertionError')modelAssertions.push(error);throw error;}}} as unknown as ModelClient;
