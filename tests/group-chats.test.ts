...
test('each human or bot message broadcasts to all others, directly and concurrently without a judge',async t=>{
  ...
  const fx=fixture(t,(run,messages,tools,signal)=>{
    assert.ok(tools.some(tool=>tool.function.name==='group_read'));assert.ok(!tools.some(tool=>tool.function.name==='group_wake'));assert.ok(!messages[0].content?.includes('只返回 JSON'));
