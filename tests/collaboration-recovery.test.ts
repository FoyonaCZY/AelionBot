...
test('a failed group task resumes for its original member and posts one final group reply',async t=>{
  ...
  const failed=f.store.data.runs.find(run=>run.botId===f.a.id&&run.status==='failed')!;assert.ok(failed?.groupOrigin);assert.equal(f.operations(),1);f.store.data.model.contextTokens=128000;resumed=true;f.groups.retryRun(failed);
