...
function footer(site,logo){
  return `<footer class="site-footer"><div class="section-shell footer-inner"><a href="/" aria-label="AelionBot 首页"><img src="${escapeHtml(logo)}" width="144" height="41" alt="AelionBot"></a><p>好想法，值得一起做。</p><nav aria-label="页脚导航"><a href="/blog/">博客</a><a href="${escapeHtml(site.repository)}" target="_blank" rel="noopener noreferrer">GitHub</a><a href="${escapeHtml(site.repository)}/releases" target="_blank" rel="noopener noreferrer">版本更新</a><a href="${escapeHtml(site.repository)}/issues" target="_blank" rel="noopener noreferrer">反馈建议</a></nav><span>© ${new Date().getFullYear()} AelionBot</span></div></footer>`;
}
...
export function renderBlogDocument({template,site,logo,posts,post,notFound=false,preview=false}){
  ...
  const description=post?.description||'关于 Agent、工程实践和 AelionBot 的开发笔记。';
